...
> 状态：调研与方案冻结稿；2026-08-10 更新。T0 自动视觉/GPU 基线、T1 全局色调、T2-A 主视图描边与共享语义契约及 T2-B1 的 M3/M11、共享鸟/当前弹弓和三类画中画接线均已通过验收；无 M7 的 T2-C1 已形成当前基线。M7 在 Beam-C3 完成前继续 fail closed。T3-A0/A1/A2/A3 已形成非 M7 当前材质基线，T3-B/C 等待 M7。T4-A0 环境合同/诊断与 T4-A1 球心大气/确定性程序化 HDR 星场已完成用户验收；T4-A2.1 云岛形态与表面基线已在实现版本 44 上完成用户 PIE，当前进入 T4-A2.2 多云团语义，随后为 A2.3 有界穿云、A2.4 消费端与性能冻结。T4-A3 环境 Profile 与 T4-B 联合校色尚未开始。
>
...
- **T4-A2**：原生 Volumetric Cloud 与双层全屏云壳已因极小主星尺度和云上视角失败而退役；现行路线按 **A2.1 云岛形态与表面基线（已验收）→ A2.2 多云团语义 → A2.3 有界穿云 → A2.4 消费端与性能冻结** 推进；R0/R1/B3B6/v44 只作为技术历史和实现身份；
