...
> 状态：产品路线已确认；M11.0 前置收口已完成编译、自动化和固定 Seed 独立进程基线，待 PIE 视觉/交互验收；M11 轨道求解器、终局表现与 PIE 验收尚未进入实施。
>
...
本稿确定 M11 的算法方向、数据契约、玩家反馈与正式验收方法。M11.0 必须先完成终局槽、配方和卫星隔离收口，详见 [M11.0 终局前置收口](M110PreFinaleClosureDesign.md)。
...
进入作用圈时，确定性自然飞越克隆必须在同一个 `AssistReferenceRadius` 上取得：
...
作用圈入口速度不能直接冒充双曲线余速。以固定行星中心参考系的入口二体能量计算：
...
当 \(v_\infty^2\le v_{\infty,\min}^2\)、无法稳定拟合渐近方向或未能再次穿出参考球时，本次遭遇是 `AssistInvalidHyperbola` 或 `PlanetCaptured`，不得继续套用助推公式。
...
进入作用圈时，求解器克隆当前状态，先模拟一次“中心引力 + 固定行星自然偏转”，取得自然近星点、B-plane 偏差、出口状态和预计停留子步。`q_i` 在这次确定性入口规划中锁定，不随渲染帧或修正后的瞬时位置反复跳变。
...
- 作用圈进入/退出、解析球碰撞、\((\mathbf r-\mathbf C_i)\cdot\mathbf v=0\) 最近点和 UFO 命中均在线段内求交；临近根时使用固定最大深度的确定性细分和固定迭代二分，不依赖渲染帧时间；
- 使用双精度状态，太空段关闭空气阻力；
...
- [ ] 同一初态、场景版本和求解器版本重复运行，点列、事件序列和结果 Hash 一致。
- [ ] 预演和实际飞行使用同一求解器；30/60/120 FPS 下助推事件、终止原因和 UFO 命中结果一致。
- [ ] 预演点与实际运动的最大偏差不超过最小碰撞半径的 `0.1%`，且不会随三次助推单调累积。
- [ ] 作用圈、碰撞和命中使用 swept 检测，高速时不穿星或穿过 UFO。
- [ ] 太空段无空气阻力；保守段做步长减半收敛测试，能量漂移处于批准阈值内。
...
| `M11-A` | 无 World/Actor 的纯数据求解器与测试夹具；中心束缚、单颗自然偏转、虚拟动量换能、步长收敛、确定性与助推消融自动化 | 地图、实飞接管、美术、终局演出 |
...
默认实施顺序是先完成 [M11.0 前置收口](M110PreFinaleClosureDesign.md)，再进入 `M11-A` 的纯数据测试夹具与确定性自动化。
