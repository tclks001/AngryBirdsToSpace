...
- 在编辑器中直接拖入并变换 Floor、树 HISM、石头 HISM、四材质砖和四档完整弹弓；
...
1. 分别拖入 Tree HISM、Rock HISM、Wood/Stone/Iron/Glass Brick。
...
5. 拖入 `M7.1 Modular Building Anchor`，作为未来自动建筑生成位置标识。
...
7. 编辑器退出 PIE 后，所有手工摆放 Actor 保持原 Transform，不产生运行时 Stake/Cord 残留资产。
