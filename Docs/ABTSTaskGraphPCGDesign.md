...
- 固定 Seed 数量与顺序由阶段 Manifest 冻结；M3R-1 使用 21 个 Compatibility Seeds，M3R-7 另用 1000 Seed 认证清单。每个确定性用例至少运行两次，逻辑结果 Hash 必须一致。
