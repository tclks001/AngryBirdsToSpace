...
- 主控鸟脚步按实际球面切向位移累计。默认移动速度翻倍后，速度映射同步改为 120→1360 cm/s，步距翻倍为 880→544 cm，响度仍为 0.12→0.27，因此同等输入状态下单位时间脚步频率和音高保持不变。跟随鸟、弹弓飞行、传送和控制权交接会重置累计值，不会四鸟齐响或传送补播；
- 四轨音乐组件均按非空间、暂停期间继续播放的持久音乐注册。开局动画暂停 Gameplay 时只冻结世界玩法，不暂停或重启音乐 stem；弹弓拉弦等 Gameplay loop 仍服从暂停；
