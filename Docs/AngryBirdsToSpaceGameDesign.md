...
- 鸟群、相机与 UI：[鸟群跟随 Gameplay](BirdPartyFollowingGameplayDesign.md) · [M4 工程落地](M4BirdPartyImplementationDesign.md) · [M4 Orbit Camera](M4MultiCharacterOrbitCameraDesign.md) · [统一镜头视觉优化](ABTSCameraVisualOptimizationDesign.md) · [UI 系统](UISystemDesign.md) · [事件驱动引导系统](ABTSGuidanceSystemDesign.md) · [CuteBird 迁移与动画](CuteBirdMigrationAndAnimationDesign.md)
...
- 数值冻结与拉通：[技术演示数值冻结设计](TechnicalDemoNumericFreezeDesign.md)
...
| M3 | 主路与月度 Encounter PCG | R-0 已验收；R-1/2/3/3.1 的路线候选、六 Encounter 逻辑和普通槽场已进入集成基线。R-3.1 的通用 M5.1/M6 消费规则已通过自动化和兼容世界 PIE，但月度实体槽仍等待 R-4/R-6 唯一 Candidate。R-4.1 已有 M6/M9 标定候选作确定性预筛，仍须等待其可见 PIE 冻结、Integration 批准的生产只读预测适配器、M7 Profile Catalog 和 vNext 建筑合同；标定模型本身不构成生产 Witness。详见 [M3R 路线](M3PCGMapImprovementPlan.md)。 |
...
| M6 | 弹弓、发射与碰撞 | 两桩+弹弦组成简易弹弓；槽位朝向/间距影响预览弹道和初速。已连接弹弓进入瞄准/拉伸/万有引力预测/发射闭环；HISM 按命中提升动态代理并支持连锁破坏，黑鸟支持手动/延时爆炸，落地静默后物体冻结与队伍回归；本轮新增的厘米长度、三维净空和失败原子门已通过兼容世界 PIE。隔离标定候选从真实 Reinforced cord/pouch frame、实际相机鼠标投影平面与玩家可达 Pull 档生成离散认证网格，并用实际鸟碰撞半径统一卫星/E5 的预测与命中；标定 V0 的真实发射可见 PIE 仍待验收。玩法见 [M6 发射详稿](M6SlingshotLaunchAndImpactDesign.md)，标定边界见 [M6/M9 标定模式](M6M9SlingshotSatelliteCalibrationDesign.md)，双弦、弹珠袋和模型协议见 [M6 视觉详稿](M6SlingshotVisualPresentationDesign.md)。 |
...
| M9 | 卫星与强化弹弓 | 生成一颗 `CellTopo Sub=2`、渲染 `Sub=4` 的纯灰卫星；以 `SatelliteWindow` Seed 锚定，局部逆平方引力叠加到鸟体与预览弹道。M3 V3 保证它远离 LaunchSite 且与终局双桩轴近似共线；M11 数据端明确排除该卫星。生产表面引力比保持 `0.25`，隔离标定参数不得泄漏；背面 E5 立方体、碰撞一致预测、无光照画中画和卫星相对镜头已进入候选，自动化/runtime 门已通过，可见 PIE 仍待验收。详见 [M9 卫星详稿](M9SatelliteGravityDesign.md)与 [M6/M9 标定模式](M6M9SlingshotSatelliteCalibrationDesign.md)。 |
