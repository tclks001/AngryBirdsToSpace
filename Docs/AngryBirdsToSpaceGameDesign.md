...
| M9 | 卫星与强化弹弓 | 生成一颗 `CellTopo Sub=2`、渲染 `Sub=4` 的纯灰卫星；以 `SatelliteWindow` Seed 锚定，局部逆平方引力叠加到鸟体与预览弹道。M3 V3 保证它远离 LaunchSite 且与终局双桩轴近似共线；M11 数据端明确排除该卫星。生产表面引力比保持 `0.25`，隔离标定候选的 `0.45` 不得泄漏；本轮 deferred transform 修复已通过单列 fresh 生产 M9 回归，标定自动化/runtime 门已通过，可见 PIE 仍待验收。详见 [M9 卫星详稿](M9SatelliteGravityDesign.md)与 [M6/M9 标定模式](M6M9SlingshotSatelliteCalibrationDesign.md)。 |
