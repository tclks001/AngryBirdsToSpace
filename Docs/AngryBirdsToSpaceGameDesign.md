...
| M7.3-DAG | 递归主体承载图 PCG | 现行路线以 Macro DAG 为骨架，通过 Series/Parallel 编译 Plate/Column 并从真实碰撞反建 Contact DAG。DAG2.3 已进入生产；DAG3-A/B/C 与 DAG-4 已完成显式测试 Profile 下的静态/动态候选认证，但生产默认仍关闭，等待弱点击毁可见 PIE。详见 [总体设计](M73RecursiveSupportDAGProceduralBuildingGenerationResearch.md) 与 [DAG-4](M73DAG4SettledContactAndAttackRolloutDesign.md)。 |
