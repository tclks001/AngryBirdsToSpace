...
> 状态：迁移与资产接入设计稿。源资产已在 `C:\workspace\UE51Test\Content\CuteBird` 通过 UE 5.1 成功打开；目标项目为 UE 5.8 的 `AngryBirdsToSpace`。
>
...
当前 C++ 仍以 `UStaticMeshComponent BirdVisual` 显示占位球。因此本次资产迁移分为两个独立验收层：

1. **资产迁移层**：UE 5.1 → UE 5.8，四种外观、共享骨架、材质、纹理、Physics Asset 和动画可在目标项目无缺失打开、预览、保存。
2. **表现接入层（后续实现）**：把 `BirdVisual` 从 Static Mesh 表现升级/旁路为 Skeletal Mesh 表现，挂接统一 Animation Blueprint；不改角色移动/碰撞根组件。
...
| --- | --- | --- | --- |
| `Cutebird_IdleA` | 是 | 常驻静止、队列停止、默认待机 | Grounded Idle 主循环。 |
| `Cutebird_IdleB` | 推荐 | 长时间未输入时的第二待机变化 | 随机/定时切换，不改变移动逻辑。 |
...
### 5.1 统一 Animation Blueprint，而不是每只鸟一套

四只鸟共用 `SK_Cute_Bird_Skeleton`，因此后续只创建一个：

```text
ABP_ABTS_CuteBird
```

颜色、脸部材质和 BirdId 由每只 ABTS Bird 的外观配置决定；Animation Blueprint 只读取状态参数，不能根据 `BP_Cute_Bird_12/3/10/16` 写四份重复状态机。
...
C++ / Physics / Party 的真实状态
        ↓ 只写只读表现参数
ABP_ABTS_CuteBird 状态机、Montage、材质参数
        ↓
Skeletal Mesh 可见表现
```
...
`AABTSM1BirdCharacter` 当前创建的是 `UStaticMeshComponent BirdVisual`，并通过 `SetBirdVisualMesh(UStaticMesh*)` 设置模型；M4 设置对象也暴露 Static Mesh 入口。因此当前编译通过的球形占位只支持静态网格，CuteBird 动画迁移完成后仍不会自动播放。
...
2. 新增或替换为 `USkeletalMeshComponent BirdSkeletalVisual`，挂接到现有 Capsule；它固定 `Collision Enabled = No Collision`、`Simulate Physics = false`。
3. 使用共享 `ABP_ABTS_CuteBird`，由 C++ 只写 `Speed`、`bGrounded`、`bJumped`、`bLaunched`、`ImpactType`、`Emote` 等表现数据。
4. 在 `ABTSBirdPartySettings` 的每只鸟配置中，把现有 Static Mesh 外观入口扩展为 Skeletal Mesh + Material Override/Face Override；保留 Static Mesh/球体回退，避免旧地图或未配置资源失效。
5. 迁移后的 `BP_Cute_Bird_*` 只作为“该 BirdId 采用哪套 Material/Texture”的编辑器参考，不嵌套为 Character 的子 Pawn，也不继承其事件图。
6. 在 fresh Editor 创建新的 ABTS Bird Blueprint，验证 Components、Details 和动画预览；不得把 Native Default Subobject 改名后假定旧 Blueprint 自动兼容。

### 6.3 每只鸟的外观配置目标

| BirdId | Skeleton Mesh | Animation Blueprint | 材质来源 | 回退 |
| --- | --- | --- | --- | --- |
| Red | CuteBird 共享 Mesh | `ABP_ABTS_CuteBird` | 从 `BP_Cute_Bird_12` 记录的 Material Slot 复制 | 红色球形。 |
| Blue | 同上 | 同上 | 从 `BP_Cute_Bird_3` 复制 | 蓝色球形。 |
| Yellow | 同上 | 同上 | 从 `BP_Cute_Bird_10` 复制 | 黄色球形。 |
| Black | 同上 | 同上 | 从 `BP_Cute_Bird_16` 复制 | 黑色球形。 |

## 7. 编辑器验收顺序

### 7.1 资产迁移验收

1. 在 UE 5.8 中打开四个 `BP_Cute_Bird_*`，截图确认颜色和脸部对应身份。
2. 打开共享 Mesh，确认引用的 Skeleton 与 `SK_Cute_Bird_Skeleton` 一致。
3. 打开 `IdleA`、`Move`、`Jump`、`Fly`，在 Animation Editor 中逐一播放并确认朝向、比例、脚/翅膀没有异常。
4. 打开 `Attack`、`Damage`，决定它们是否语义适合撞击反馈；不适合则仅记录为未采用候选。
5. 保存全部已经转换的资产，关闭并 fresh 重开 UE 5.8。

### 7.2 后续 Skeletal 表现验收

1. 在 `L_ABTS_M4` 中四鸟都显示正确颜色，不再显示占位球。
2. 不按键时播放 Idle；行走时播放 Move；跳跃时播放 Jump 后转 Fly；落地后回 Idle/Move。
3. Tab/HUD 切换主控不会重置动画、丢材质、改变相机 Orbit 或破坏队列逻辑。
4. 球面跨越极点、斜坡、平面 M7.1/Chaos 测试台时，骨骼视觉的本地 Up 仍服从角色现有径向/平面视觉框架。
5. 飞行/发射中动画不改变物理轨迹；关闭动画或替换回球体不会改变弹道、碰撞或伤害结果。
...
1. 在 UE 5.1 源项目按第 4.2 节迁移四个 Blueprint、共享 Mesh/Skeleton/Physics Asset。
2. 按第 4.3 节迁移七条首版动画。
3. 在 UE 5.8 完成转换并逐项保存/预览。
4. 在 `SourceArt/Purchased/CuteBird/ReferenceScreenshots/` 保存四个源 Blueprint 的 Mesh/Material Details 截图。
5. 把迁移报告截图和实际迁移日期写入 `Docs/AIReports` 之外的第三方资产来源记录；CuteBird 是购买资产，不属于 AI 生成记录。
6. 资产迁移验收通过后，再开始单独的 Skeletal 表现代码/蓝图接入任务；不要在迁移过程中修改现有 M4 Character 的移动或碰撞组件。
