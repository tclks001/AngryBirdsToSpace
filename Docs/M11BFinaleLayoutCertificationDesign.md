...
### Rank 8 的 v2.2 全 Power 预认证早停（2026-08-01）

用户完成 Rank 8 PIE 手感验收后，以冻结
`CandidateSourceHash=0x617687274ed0c29a` 继续执行标准 C++ 全 Power 预认证。完整
`Yaw=[-18°,18°] / Pitch=[0°,60°] / Power=[0,1]` 粗网格
`2° / 3° / 0.025` 共 16359 点，得到
`F1/F2/F3/F4=282/89/4/2`、分量数 `2/2/1/1`，名义输入位于唯一 F4，
嵌套违规为 0。

全域 half-cell `1° / 1.5° / 0.0125` 共 122877 点，得到
`2140/707/29/10`、分量数 `3/2/2/4`、
`AggregateSampleHash=0xa055817aa70d24d1`。四个 F4 分量只有 `5/3/1/1` 点，
且全部位于 `Yaw=[0°,1°] / Pitch=[28.5°,34.5°] / Power=[0.9,1]`。
将包含全部 F4 种子的闭包
`Yaw=[-4°,2°] / Pitch=[19.5°,34.5°] / Power=[0.875,1]` 提升到
`0.25° / 0.375° / 0.003125` 后，共 42025 点得到
`27713/20976/1538/480`、分量数 `1/1/3/1`。因此 half-cell 的四分量是斜向
成功带被粗六邻域切断，不是四个稳定 F4 岛；Rank 8 在该局部闭包内仍保持严格
F4 单岛。

但全域 half-cell 同时显示 F1 最低 Power 为 `0.75`。为定位第一颗行星可达下界，
进一步对完整角度域和 `Power=[0.7,0.8]` 使用 `PowerStep=0.003125` 扫描 50061 点，
得到 `F1/F2=532/67`；F1 首次出现于 `Power=0.746875`，F2 首次出现于
`Power=0.76875`。F1 是 532 点单连通区域，不是离散异常点。这违反第 8.3 节
“行星①只有接近最大功率的连续区间可达”以及候选构造期约 `0.88–0.95` 的目标
下界，故本轮按正式门槛早停，不执行消融、错序/迟到旁路、Trust Region、
...
### 刚性星群远移至 Power 0.88 的可行性搜索（2026-08-01）

在 Rank 9 的同一刚性平移语义下，继续把诊断距离扩展到 120000 cm。搜索始终保持
行星①②③与 UFO 的相对位置、B-plane、虚拟动量和半径不变，只增加弹弓袋到整个
星群的共同距离。

`2° / 3° / 0.025` 全域粗扫显示：约 53000 cm 时 F1 下界一度显示为 0.875 且仍有
F4；但 `1° / 1.5° / 0.0125` half-cell 在 `51000..55000 cm` 找到近侧绕行输入，
真实 F1 下界只有 `0.8125..0.825`。继续远移后，70000 cm 粗网格的 F1 下界达到
0.90，但 F3/F4 均为空。

为排除粗角度漏采，最大功率下以 `0.5° / 0.75°` 扫描
`60000..70000 cm`、距离步长 500 cm，所有位置 F4 均为 0；随后在
`55000..60000 cm` 以 250 cm 距离步长检查，最远的稀疏 F4 出现在 59750 cm。
对 59750 cm 执行完整 half-cell 后得到
`F1/F2/F3/F4=72/43/0/0`、F1 下界 0.825，证明该稀疏 F4 也不能在更完整角度域
存活。100000 cm 以后连粗网格 F1 都为空。

结论是：只使用这一维刚性平移，无法同时满足 `MinimumF1Power>=0.88` 和非空三助推
F4。Rank 9 的 100 cm 微调继续保留，但不新增失败 Rank。若 0.88 是正式硬门，
后续至少要放宽一个条件：单独移动行星①、重新求解后续三体相对布局，或收窄
Yaw/Pitch 输入域；这些都会超出“四天体相对位置不变”的本轮约束并要求重新 PIE。

### Rank 8 径向 5900 cm 与受限角向修复候选（Rank 10，2026-08-01）

在放宽 F1 Power 正式门后，四个天体分别沿各自 Pouch 射线外移 5900 cm，并只对
下游三体做小幅角向修复：行星②、行星③、UFO 相对 Rank 8 的角偏差约为
`1.249° / 0.717° / 1.259°`。最大功率局部 1025 点闭包为
`481→249→124→62`，条件比例约 `0.469→0.518→0.498→0.500`；完整 ScreenAim
5000 点为 `392→115→27→15`。42025 点局部三维闭包为
`21997→11375→5149→2569`，四层均为单一六邻域分量。

该布局以 `abts.M11.CandidateRank 10` 加入 Editor-only 列表末尾，冻结新的成功岛
代表输入 `Yaw=-1.25° / Pitch=30.375° / Power=1`。身份为
`Source=0x2b06db2cf348d75f / Request=0xa1d91650dc3d3f36 /
Result=0x99012cedf3d01c06 / ScoreEvidence=0x22c3f67f46d49e70`。Rank 10 仍是
...
### Rank 10 终端局部修复边界（2026-08-01）

为保留 Rank 10 的三次偏转全貌，本轮先只开放 UFO 合格拦截中心和半径。确定性
Halton 搜索依次执行 512 组 `±3000 cm`、512 组 `±6000 cm` 和围绕最好中心的
256 组 `±1200 cm` 局部构造；每轮先做稀疏三维 Power 初筛，再把固定排序的前列
候选提升到 `Yaw=0.25° / Pitch=0.375° / Power=0.00625` 的 33825 点闭包。

纯 UFO 最好中心偏移为 `(-2320.312,-897.119,-3657.600) cm`、半径 6000 cm：
`F=12242/6483/3220/417`、分量数 `1/1/1/7`、主 F4 为 400 点、碎片 17 点，
名义输入保留且 `EarlyTargetHit=0`。它消除了原 Rank 10 的 857 个提前命中，但没有
合并 F4。固定该中心把半径从 6000 cm 以 125 cm 步长缩到 4500 cm 时，主岛先于
碎片消失：5500 cm 已变为 `F4=214 / 8 分量 / 主岛195`，4500 cm 时名义输入也
退出 F4。沿最好方向把中心距离扫到 9000 cm、半径扫 `4500..6000 cm`，最好仍为
7 个分量。到达速度锥 `5°..120°` 也只能裁点；30°以上不改变 7 分量，以下反而
重新产生 EarlyTargetHit。

作为“不改变前两段轨迹”的最后检查，只开放行星③ B-plane T/R `±600 cm` 和
Sigma `±12%`，完成 256 组初筛及 24 组精筛。最好参数为
`BPlaneDelta=(-450,-66.667) cm / SigmaScale=1.072`，结果仍为
`F4=418 / 7 分量 / 主岛401 / 碎片17`。因此，多岛来自上游输入经行星③出口映射
到终端球时的多支结构，不是 UFO 球心、半径或简单终端方向门能局部修复的问题。

本轮不新增 Rank，也不覆盖 Rank 10。若必须得到严格单岛，需要重新开放行星③
虚拟动量/中心，甚至行星②上游映射并重新搜索；这会改变第三次偏转或更早轨迹，
...
参数 `mu` 乘 `s³`。固定积分时间步仍为 `1/120 s`，因为本实验保持飞行时间不变；
若把时间步也乘 `s`，采样率会从 120 Hz 降至约 17.45 Hz，并不满足动力学相似性，
还会放大 swept 事件漏检风险。

主星视觉/碰撞半径、中心和 `mu` 必须继续匹配 M3 世界与发射表面，不能随深空布局
放大；只扩展了主星外层模拟域，使放大的远端轨迹不会被提前裁掉。独立几何 UFO
仍保持原视觉/接触尺度，本轮只按要求放大 6000 cm 合格拦截包络到 41250 cm。
因此这不是全四体的严格相似变换，必须重新求解验证，不能由比例公式直接批准。
...
| 局部分量数 | `1/1/1/1` | `1/1/2/2` |
| 冻结名义输入属于 F4 | 是 | 否 |
| 代表 F4 | `(-1.25°,30.375°,1)` | `(-2.5°,26.625°,1)` |
...
局部满功率域的上游条件比例接近 `1/2`；随后固定 F1/F2，只先搜索 UFO 对既有 F3
像的截取；当仅移动 UFO 不能形成单岛时，才开放一个受限的行星③/UFO 联合邻域。
因此每一步只重算当前及其下游前缀，不回退已经合格的上游链路。
...
| --- | ---: | ---: | ---: | ---: |
| 样本数 | 531 | 258 | 84 | 30 |
| 条件比例 | 0.5180 | 0.4859 | 0.3256 | 0.3571 |
| 六邻域分量 | 1 | 1 | 1 | 1 |

5000 点无偏满功率 ScreenAim 为 `616→137→21→6`，对应
`F1/S0=0.1232`、`F2/F1=0.2224`、`F3/F2=0.1533`、`F4/F3=0.2857`，均在
v2.1 冻结的 `[0.08,0.55]` 手感门内。四级 Hull 面积依次为
`356.2407 / 78.5821 / 10.3939 / 1.2912 deg²`，F4 Hull 跨度为
`2.2676° × 3.5117°`。代表 F4 输入为 `(-1.75°,26.25°,1)`，飞行
`30.680732 s`，三段影响区时长 `6.4340/5.9891/5.0716 s`，三次偏转
`0.407789/0.522593/1.335500 rad`。
