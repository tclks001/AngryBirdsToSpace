...
>
> 状态：DAG-1、DAG-2 与 DAG2.3 已落地并接管球面 TaskGraph 普通建筑生产链；DAG3-A/B/C、DAG-4、DAG5-A/B v1 已实现并保持默认关闭；DAG5-B v2 的 Shape Grammar + WFC 复杂语义轮廓已完成人工读形。当前正式演进改用[长条梁式结构路线](M73BeamBlockStructuralGenerationResearch.md)，由 [Beam-A](M73BeamAStructuralIRPreviewDesign.md) 先建立 Bay / Joint / Member / Assembly IR。Plate 拉伸方案和 DAG5-C～E 暂不推进。完整阶段状态、历史方案和模块复用清单见 [M7 建筑系统文档导航](M7BuildingDevelopmentRoadmap.md)。
>
