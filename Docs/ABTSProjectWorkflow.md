...
- 默认下一步：Integration 已落地隔离 M6/M9 标定候选；标定 GameMode 蓝图现为三档曲线和卫星预设的唯一人工参数源，Slingshot Camera 蓝图现为相机构图的唯一人工参数源，几何 CVar/命令行与 Catalog→Camera 反写已废弃。E5 碰撞一致预测、背面画中画和卫星镜头候选已落地，蓝图入口的 fresh 6/6、runtime smoke、生产 M9 与 M11 隔离回归已通过；完成可见 PIE 手感验收后才能冻结 V0。M7 并行推进 DAG5-B/C，M3 可推进 R-5；冻结的可移植 Launch/Preset 身份、Integration 批准的生产只读适配器与 M7 目录齐备后，由 M3R-4 选出唯一 Candidate，再接通 R-3.1 月度实体槽与 R-6 六栋世界。
