...
- M11：v1 的 M11.0/A/B/C 是生产基线；A/B/C v2.1 的 Core、Editor-only 候选和交互表现已进入 `master`。M3R-5.2 道路末端帧已在 `L_ABTS_M11` 接通 M5.1 双槽与 M11 3+1 表现，自动化和 fresh NullRHI 通过，待 Visible PIE；候选仍为 `NOT CERTIFIED`，不能替换 v1 默认值。
