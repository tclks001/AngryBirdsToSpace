...
- Integration 世界子系统每 `0.1 s` 只读查询 M11 公开的 `GetFinaleEnvironmentStage()`：`GroundLaunch` 保持配置基线（生产默认 `GroundDay`）；`AtmosphereTransition` 继续消费既有按相机高度计算的连续空间混合，混合尚未完成时保持 GroundDay，达到 1 后立即选择 `FinaleSpace`；`DeepSpace` 继续确认 FinaleSpace，`Recovering` 立即回到配置基线。这样第一颗行星从进入构图起就是同一太空光照，不会在 `AssistEnter(1)` 边界整套换档。缺环境快照、相机或非法数据时不提前晋升并 fail closed。该装配不写 M11 私有状态，也不修改积分器、轨迹、镜头、PIP 或演出；
