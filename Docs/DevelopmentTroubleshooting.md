...
>
> 导航：[主设计稿](AngryBirdsToSpaceGameDesign.md) · [M3 地形表现](M3TaskGraphTerrainPresentationDesign.md) · [Chaos 刚体移动](ChaosRigidBodyMovementDesign.md) · [M7.3-A 稳定建筑](M73AStableBlockBuildingImplementationDesign.md) · [M7.3-B 弱点与难度](M73BWeakPointAndDifficultyDesign.md) · [M7.3-B2 结构失效验证](M73B2StructuralWeaknessAndFailureValidationDesign.md) · [M7.3-DAG 递归主体建筑](M73RecursiveSupportDAGProceduralBuildingGenerationResearch.md) · [M7.3-DAG-2 空间布局](M73DAG2SpatialLayoutAndModuleCompilationDesign.md)
...
| `TwinTowerBridge` 在 `MaxExpansionDepth=1` 时出现 `DAGNoFeasibleSupport:0` | 双塔基准已沿 X 切分；Seed 7301 的下一次递归又选择 `ParallelSplit`，最外侧子 Plate 与中部 Plate 的有效投影小于双柱所需的接触宽度。`0` 是第一个无法获得候选支撑的 Macro Id，不是数组或 Actor 损坏 | 基准双塔先设 `MaxExpansionDepth=0`、`ExpansionStepBudget=0`；若要保留一层递归，则固定 `SeriesRuleWeight=1`、`ParallelRuleWeight=0`、`MaxExpansionDepth=1`，避免对横向塔臂再次 ParallelSplit。未来由 Beam/Span 原语支持这种真正的跨距分支 | Twin 基准可用；一层纯 Series 细分时每个非 Ground Plate 都得到 `Plate→Column→Plate` 支撑；不靠强行加旁路柱通过 |
