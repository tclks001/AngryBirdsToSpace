...
| 三套环境 Profile 已有参数，但普通世界仍只读诊断 CVar；若直接让月面 PIP 改用 `SatelliteGuide`，会把已验收的月面实体光照和 Tone 一起改掉 | 一个 `FABTSStylizedViewPolicy::Profile` 同时承担实体表面与环境背景，且主世界没有玩法状态到 Profile 的消费路由。参数存在不等于正式装配；直接在 M11 私有代码切 CVar还会使退出/失败恢复与其他视图耦合 | v65 将视图策略拆为 Surface `Profile` 与 `EnvironmentProfile`；月面保持 GroundDay 表面并只让空背景消费 SatelliteGuide。Integration 世界子系统只读 M11 `IsFinaleActive()`，活动期发布 FinaleSpace 环境快照，结束后重新解析配置基线；主视图优先消费该快照，不写 M11 求解/演出状态 | `TOON-T4A32-001`：纯合同冻结三套 Actor 可见策略、月面双 Profile、终局优先级及 inactive 回切。ForceUnity/fresh Toon 后，PIE 依次验证普通世界、月面 PIP、终局进入/退出/失败和 Rank11 AVI；异常进程/地图切换恢复仍属于 A3.3，不能由正常轮询回切冒充 |
