...
| 切换主控后新鸟看似自行滑行，空格暂时无效，并伴随约 4 秒 `HandoffDiag` | `HandoffDiag` 是每次 Tab 后主动开启的固定 4 秒诊断窗口，不是震荡触发器。完整日志显示问题只发生在切到 `ForceGround=0` 的空中跟随鸟时：旧切换逻辑将它的速度清零，却保留半空位置，随后无输入也会沿径向重力持续坠落；这在球面视角下看似朝固定方向自主移动，空中自然无法跳跃。另有贴地 Support 反弹曾产生正径向速度并误触发 Airborne，污染跟随跳跃事件。 | 普通鸟群切换采用确定性落地交接：新主控若未接地，按当前位置的 CellTopo 派生连续地表半径对齐，并刷新稳定 Grounded；已接地则只清动量。离任主控若仍在空中，只清输入权限、保留合法飞行速度。接地状态中径向速度被约束为零，贴地高度由 `BuildGroundFollowingDelta` 唯一负责；已有 Grounded 不再因 Support 的短暂正速度退出，显式 `NotifyJump` 仍可离地。 | 切到空中跟随鸟时日志出现 `[ABTS][M4][HandoffGround] Snapped=1 ... AfterGround=1`，紧随其后的快照必须 `ForceGround=1 / ForceVel=0`；切到已落地鸟时 `Snapped=0`。静止接地不再反复生成 JumpFollow；4 秒后 HandoffDiag 正常停止。 |
