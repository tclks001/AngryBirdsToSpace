...
namespace
{
	...
	bool IsIntegerMultipleOfBlock(const double Value)
	{
		const double Units = Value / static_cast<double>(BlockUnitsCM);
		return FMath::IsNearlyEqual(Units, FMath::RoundToDouble(Units),
		...
		FString& OutDiagnostic)
	{
		...
		Probe.LocalEnd = End;
		const FBox Bounds = PlannedMemberBounds(Probe);
		const FVector Dimensions = Bounds.GetSize();
		const bool bAxisAligned =
			(Axis == EABTSM73BeamAFrameAxis::X
				&& FMath::IsNearlyEqual(Start.Y, End.Y, GeometryToleranceCM)
				&& FMath::IsNearlyEqual(Start.Z, End.Z, GeometryToleranceCM))
			|| (Axis == EABTSM73BeamAFrameAxis::Y
				&& FMath::IsNearlyEqual(Start.X, End.X, GeometryToleranceCM)
				&& FMath::IsNearlyEqual(Start.Z, End.Z, GeometryToleranceCM))
		...
		FString& OutError)
	{
		...
		const double Maximum = Axis == EABTSM73BeamAFrameAxis::Z || bLongSpanMember
			? 720.0 : 648.0;
		FString UnitizedDiagnostic;
		if (!FMath::IsFinite(Length)
			|| Length + KINDA_SMALL_NUMBER < BlockUnitsCM
			|| Length > Maximum + KINDA_SMALL_NUMBER
			|| !ValidateUnitizedMemberGeometry(Plan, ComponentId, Axis,
				Start, End, UnitizedDiagnostic))
		{
			OutError = FString::Printf(
				TEXT("BeamC3V3MemberGeometryInvalid:Kind=%d:Axis=%d:Length=%.3f:Maximum=%.3f:%s"),
				static_cast<int32>(SkeletonKind), static_cast<int32>(Axis),
				Length, Maximum, *UnitizedDiagnostic);
			return false;
		}
		...
		case ESkeletonMemberKind::GroundSillCourse:
		case ESkeletonMemberKind::ExteriorPost:
		case ESkeletonMemberKind::GroundExteriorPost:
			Member.ProducedStage = EABTSM73BeamC3GenerationStage::CommonExteriorFrame;
			break;
		case ESkeletonMemberKind::FloorCourse:
		case ESkeletonMemberKind::FloorInfillCourse:
		case ESkeletonMemberKind::StyleInfillCourse:
		case ESkeletonMemberKind::FacadeToTopSeat:
		case ESkeletonMemberKind::FacadeToTopPost:
	...
	void AppendMemberCanonical(FString& Text, const FPlannedMember& Member)
	{
		...
		{
			Text += FString::Printf(TEXT(":P2%d"), Parent);
		}
		Text += FString::Printf(TEXT(":EF%d:EC%d:GS%d:LF%d:UF%d"),
			Member.ExteriorFrameId, Member.ExteriorColumnId,
			Member.GroundSillSegmentId,
			Member.LowerExteriorFrameId, Member.UpperExteriorFrameId);
		...
		FString& OutError)
	{
			...
			for (const FResolvedFacadeEnvelopeVolume* Candidate : FacadeVolumes)
			{
				...
				for (int32 Course = FirstCourse; Course < LastCourseExclusive; ++Course)
				{
					...
					for (const uint8 FaceMask : Stage2FaceBits)
					{
						...
						const int32 AxisIndex = bXAxis ? 0 : 1;
						const int32 TangentIndex = bXAxis ? 1 : 0;
						const double RawFacadeCM = bPositive
							? Candidate->LocalBounds.Max[AxisIndex]
							: Candidate->LocalBounds.Min[AxisIndex];
						const double FacadeCM = bPositive
							? CellFaceAtOrBelow(RawFacadeCM)
							: CellFaceAtOrAbove(RawFacadeCM);
						...
						for (int32 CutIndex = 0; CutIndex + 1 < Cuts.Num(); ++CutIndex)
						{
							...
							Witness.Z = Z;
							Witness[AxisIndex] = RawFacadeCM + (bPositive ? 1.0 : -1.0)
								* GeometryToleranceCM * 2.0;
							...
							if (bOutsideEmpty)
							{
								const double QuantizedTangentMinimum =
									CellFaceAtOrAbove(Cuts[CutIndex]);
								const double QuantizedTangentMaximum =
									CellFaceAtOrBelow(Cuts[CutIndex + 1]);
								if (QuantizedTangentMaximum - QuantizedTangentMinimum
									< BlockUnitsCM - GeometryToleranceCM)
								{
									continue;
								}
								...
								Span.TangentMinimumCM = QuantizedTangentMinimum;
								Span.TangentMaximumCM = QuantizedTangentMaximum;
							}
						}
					}
				}
			...
			uint16& OutRejectMask)
		{
			...
			for (const FResolvedFacadeEnvelopeVolume* Target : Targets)
			{
				...
				// Structural members use an inscribed 36 cm lattice even when the
				// semantic WFC envelope carries continuous bounds.  The outer member
				// face must never be rounded through the authored silhouette.
				const double FacadeCM = bPositive
					? CellFaceAtOrBelow(RawFacadeCM)
					: CellFaceAtOrAbove(RawFacadeCM);
		...
		for (int32 MemberIndex = Stage1MemberCount; MemberIndex < Plan.Members.Num(); ++MemberIndex)
		{
			...
			const double ExpectedEndpoint = Target == nullptr ? 0.0
				: bPositive
					? CellFaceAtOrBelow(Target->LocalBounds.Max[AxisIndex])
					: CellFaceAtOrAbove(Target->LocalBounds.Min[AxisIndex]);
			Plan.Summary.CouplingEndpointViolationCount += Target == nullptr
				|| FMath::Abs(Endpoint - ExpectedEndpoint)
					> GeometryToleranceCM ? 1 : 0;
	...
	bool AppendStage3CommonExteriorFrame(
		const FABTSM73BeamD0ResolvedProfile& Profile,
		FPlan& Plan,
		FString& OutError)
	{
		const double Stage3StartSeconds = FPlatformTime::Seconds();
		const int32 Stage2MemberCount = Plan.Members.Num();
		const int64 Stage2GeometryHash = Plan.Summary.FinalGeometryHash;
		Plan.Summary.Stage3InputGeometryHash = Stage2GeometryHash;
		Plan.Summary.CommonExteriorFrameCount = 0;
		Plan.Summary.GroundSillLoopCount = 0;
		Plan.Summary.GroundSillSegmentCount = 0;
		Plan.Summary.EmittedGroundSillSegmentCount = 0;
		Plan.Summary.ReusedGroundSillSegmentCount = 0;
		Plan.Summary.GroundSillConflictOmissionCount = 0;
		Plan.Summary.ExteriorColumnCount = 0;
		Plan.Summary.ExteriorColumnSegmentCount = 0;
		Plan.Summary.GroundExteriorColumnCount = 0;
		Plan.Summary.GroundExteriorColumnSegmentCount = 0;
		Plan.Summary.GroundExteriorColumnConflictOmissionCount = 0;
		Plan.Summary.EmittedExteriorFrameCount = 0;
		Plan.Summary.ReusedExteriorFrameCount = 0;
		Plan.Summary.Stage3AnchorBandWithoutFrameCount = 0;
		Plan.Summary.Stage3FrameDownwardConnectionViolationCount = 0;
		Plan.Summary.Stage3CrossPartitionColumnCount = 0;
		Plan.Summary.Stage3ParentViolationCount = 0;
		Plan.Summary.Stage3ClampViolationCount = 0;
		Plan.Summary.Stage3ColumnFrameViolationCount = 0;
		Plan.Summary.Stage3FacadeFitViolationCount = 0;
		Plan.Summary.Stage3PlanHash = 0;
		Plan.Summary.Stage3FrameMilliseconds = 0.0;
		Plan.Summary.Stage3ColumnMilliseconds = 0.0;
		Plan.Summary.Stage3StaticDAGMilliseconds = 0.0;
		Plan.Summary.Stage3TotalMilliseconds = 0.0;
		Plan.CommonExteriorFrames.Reset();
		Plan.GroundSillSegments.Reset();
		Plan.ExteriorColumns.Reset();
		...
		auto SpanAt = [](const FFacadePartitionPlan& Partition,
			const int32 Course, const double Tangent)
		{
			return Partition.CourseSpans.FindByPredicate(
				[Course, Tangent](const FFacadePartitionCourseSpan& Span)
				{
					return Span.CourseIndex == Course
						&& Tangent >= Span.TangentMinimumCM - GeometryToleranceCM
						&& Tangent <= Span.TangentMaximumCM + GeometryToleranceCM;
				});
		};
		auto PenetratesExisting = [&Plan](const FPlannedMember& Probe)
		{
			const FBox ProbeBounds = PlannedMemberBounds(Probe);
			for (const FPlannedMember& Existing : Plan.Members)
			{
		...
		for (const FFacadeHeightAnchorBand& Band : Plan.FacadeHeightAnchorBands)
		{
			const FFacadePartitionPlan* Partition = FindPartition(Band.FacadePartitionId);
			if (Partition == nullptr
				|| !Plan.Members.IsValidIndex(Band.LowerMemberIndex)
				|| !Plan.Members.IsValidIndex(Band.UpperMemberIndex))
			{
				++Plan.Summary.Stage3ParentViolationCount;
				++Plan.Summary.Stage3AnchorBandWithoutFrameCount;
				continue;
			}
			const FPlannedMember& Lower = Plan.Members[Band.LowerMemberIndex];
			const FPlannedMember& Upper = Plan.Members[Band.UpperMemberIndex];
			if (Lower.ProducedStage != EABTSM73BeamC3GenerationStage::CouplingCourses
				|| Upper.ProducedStage != EABTSM73BeamC3GenerationStage::CouplingCourses
				|| Lower.AnchorBandId != Band.AnchorBandId
				|| Upper.AnchorBandId != Band.AnchorBandId
				|| Lower.Axis != Upper.Axis
				|| Upper.CourseIndex != Lower.CourseIndex + 2)
			{
				++Plan.Summary.Stage3ParentViolationCount;
				++Plan.Summary.Stage3AnchorBandWithoutFrameCount;
				continue;
			...
			const int32 FrameCourse = Band.BaseCourseIndex + 1;
			FFacadePartitionCourseSpan SyntheticSpan;
			const FFacadePartitionCourseSpan* Span = SpanAt(
				*Partition, FrameCourse, Band.TangentCoordinateCM);
			if (Span == nullptr)
			{
				// The clamp itself proves the band crosses this facade. At a volume
				// boundary the semantic face ledger can omit only the middle course;
				// inherit either adjacent clamp course rather than dropping the frame.
				Span = SpanAt(*Partition, Band.BaseCourseIndex,
					Band.TangentCoordinateCM);
			}
			if (Span == nullptr)
			{
				Span = SpanAt(*Partition, Band.BaseCourseIndex + 2,
					Band.TangentCoordinateCM);
			}
			if (Span == nullptr)
			{
				SyntheticSpan.CourseIndex = FrameCourse;
				SyntheticSpan.SourceVolumeId = Lower.TargetFacadeSourceVolumeId;
				// The paired Stage-2 courses are physical proof of one complete
				// clamp cell even when the semantic face ledger has no middle-course
				// span.  Materialize that exact voxel instead of representing it as a
				// zero-width point that later requires an off-lattice expansion.
				SyntheticSpan.TangentMinimumCM = Band.TangentCoordinateCM
					- BlockUnitsCM * 0.5;
				SyntheticSpan.TangentMaximumCM = Band.TangentCoordinateCM
					+ BlockUnitsCM * 0.5;
				Span = &SyntheticSpan;
			}
			// Stage 2 has already reduced semantic spans to complete 36 cm cells.
			// Consume only those physical cells here; a synthetic middle-course span
			// above materializes the one proven clamp cell explicitly.
			const double AvailableMinimum = CellFaceAtOrAbove(
				Span->TangentMinimumCM);
			const double AvailableMaximum = CellFaceAtOrBelow(
				Span->TangentMaximumCM);
			const double AvailableLength = AvailableMaximum - AvailableMinimum;
			if (AvailableLength < BlockUnitsCM - GeometryToleranceCM)
			{
				++Plan.Summary.Stage3FacadeFitViolationCount;
				++Plan.Summary.Stage3AnchorBandWithoutFrameCount;
				continue;
			}
			const double FrameLength = FMath::Min(
				AvailableLength, MaximumHorizontalUnits * static_cast<double>(BlockUnitsCM));
			double RunMinimum = FMath::Clamp(
				Band.TangentCoordinateCM - FrameLength * 0.5,
				AvailableMinimum, AvailableMaximum - FrameLength);
			double RunMaximum = RunMinimum + FrameLength;
			if (Band.TangentCoordinateCM - RunMinimum
				< BlockUnitsCM * 0.5 - GeometryToleranceCM)
			{
				RunMinimum = FMath::Max(AvailableMinimum,
					Band.TangentCoordinateCM - BlockUnitsCM * 0.5);
				RunMaximum = RunMinimum + FrameLength;
			}
			if (RunMaximum - Band.TangentCoordinateCM
				< BlockUnitsCM * 0.5 - GeometryToleranceCM)
			{
				RunMaximum = FMath::Min(AvailableMaximum,
					Band.TangentCoordinateCM + BlockUnitsCM * 0.5);
				RunMinimum = RunMaximum - FrameLength;
			}
			RunMinimum = CellFaceAtOrAbove(RunMinimum);
			if (RunMinimum + FrameLength
				> AvailableMaximum + GeometryToleranceCM)
			{
				RunMinimum = AvailableMaximum - FrameLength;
			}
			RunMaximum = RunMinimum + FrameLength;

			const bool bNormalX = Lower.Axis == EABTSM73BeamAFrameAxis::X;
			const bool bPositiveFace = Band.FaceMask == PositiveX
				|| Band.FaceMask == PositiveY;
			const double FrameNormalCoordinate = Band.FacadeCoordinateCM
				+ (bPositiveFace ? -1.0 : 1.0) * BlockUnitsCM * 0.5;
			const EABTSM73BeamAFrameAxis FrameAxis = bNormalX
				? EABTSM73BeamAFrameAxis::Y : EABTSM73BeamAFrameAxis::X;
			...
			const double Z = GroundZ + (FrameCourse + 0.5) * BlockUnitsCM;
			const FVector Start = bNormalX
				? Position(FrameNormalCoordinate, RunMinimum, Z)
				: Position(RunMinimum, FrameNormalCoordinate, Z);
			const FVector End = bNormalX
				? Position(FrameNormalCoordinate, RunMaximum, Z)
				: Position(RunMaximum, FrameNormalCoordinate, Z);
			FPlannedMember Probe;
			Probe.Axis = FrameAxis;
			Probe.LocalStart = Start;
			Probe.LocalEnd = End;
			int32 MemberIndex = INDEX_NONE;
			bool bReusesExistingMember = false;
			EABTSM73BeamAFrameAxis ResolvedFrameAxis = FrameAxis;
			double ResolvedRunMinimum = RunMinimum;
			double ResolvedRunMaximum = RunMaximum;
			if (PenetratesExisting(Probe))
			{
				// A setback or neighbouring child can already own the exact middle
				// course required by this Stage-2 clamp.  That collinear core rail is
				// physical frame material; adopting it is both stronger and legal,
				// whereas emitting a duplicate would create positive-volume overlap.
				const FBox ProbeBounds = PlannedMemberBounds(Probe);
				const double RequiredMinimum = Band.TangentCoordinateCM
					- BlockUnitsCM * 0.5;
				const double RequiredMaximum = Band.TangentCoordinateCM
					+ BlockUnitsCM * 0.5;
				auto MakeFrameProbe = [bNormalX, FrameNormalCoordinate, Z, FrameAxis](
					const double Minimum, const double Maximum)
				{
					FPlannedMember Candidate;
					Candidate.Axis = FrameAxis;
					Candidate.LocalStart = bNormalX
						? Position(FrameNormalCoordinate, Minimum, Z)
						: Position(Minimum, FrameNormalCoordinate, Z);
					Candidate.LocalEnd = bNormalX
						? Position(FrameNormalCoordinate, Maximum, Z)
						: Position(Maximum, FrameNormalCoordinate, Z);
					return Candidate;
				};
				// A long proposal may cross an unrelated core farther along the
				// facade even though the 36 cm cell actually clamped by this band is
				// clear. Preserve the anchor and grow only through collision-free
				// neighbouring cells instead of discarding the entire frame.
				const FPlannedMember MinimumProbe = MakeFrameProbe(
					RequiredMinimum, RequiredMaximum);
				if (!PenetratesExisting(MinimumProbe))
				{
					ResolvedRunMinimum = RequiredMinimum;
					ResolvedRunMaximum = RequiredMaximum;
					bool bExpanded = true;
					while (bExpanded)
					{
						bExpanded = false;
						const double CandidateMinimum = FMath::Max(RunMinimum,
							ResolvedRunMinimum - BlockUnitsCM);
						if (CandidateMinimum < ResolvedRunMinimum - GeometryToleranceCM
							&& !PenetratesExisting(MakeFrameProbe(
								CandidateMinimum, ResolvedRunMaximum)))
						{
							ResolvedRunMinimum = CandidateMinimum;
							bExpanded = true;
						}
						const double CandidateMaximum = FMath::Min(RunMaximum,
							ResolvedRunMaximum + BlockUnitsCM);
						if (CandidateMaximum > ResolvedRunMaximum + GeometryToleranceCM
							&& !PenetratesExisting(MakeFrameProbe(
								ResolvedRunMinimum, CandidateMaximum)))
						{
							ResolvedRunMaximum = CandidateMaximum;
							bExpanded = true;
						}
					}
				}
				...
				{
				const FBox MinimumProbeBounds = PlannedMemberBounds(MinimumProbe);
				double BestCoveredLength = -1.0;
				for (int32 ExistingIndex = 0;
					ExistingIndex < Plan.Members.Num(); ++ExistingIndex)
				{
					const FPlannedMember& Existing = Plan.Members[ExistingIndex];
					if (Existing.Axis == EABTSM73BeamAFrameAxis::Z
						|| Existing.CourseIndex != FrameCourse)
					{
						continue;
					}
					const FBox ExistingBounds = PlannedMemberBounds(Existing);
					const bool bSameSlab = FMath::Abs(ExistingBounds.Min.Z
						- ProbeBounds.Min.Z) <= GeometryToleranceCM
						&& FMath::Abs(ExistingBounds.Max.Z
							- ProbeBounds.Max.Z) <= GeometryToleranceCM;
					const bool bCoversClampCell =
						ExistingBounds.Min.X <= MinimumProbeBounds.Min.X + GeometryToleranceCM
						&& ExistingBounds.Max.X >= MinimumProbeBounds.Max.X - GeometryToleranceCM
						&& ExistingBounds.Min.Y <= MinimumProbeBounds.Min.Y + GeometryToleranceCM
						&& ExistingBounds.Max.Y >= MinimumProbeBounds.Max.Y - GeometryToleranceCM;
					const bool bOwnsFacadeJunction =
						(Existing.SkeletonKind == ESkeletonMemberKind::FacadeCourse
							|| Existing.ProducedStage
								== EABTSM73BeamC3GenerationStage::CouplingCourses)
						&& BoxesPenetrate(MinimumProbeBounds, ExistingBounds);
					const double ExistingMinimum = FrameAxis
						== EABTSM73BeamAFrameAxis::X
						? ExistingBounds.Min.X : ExistingBounds.Min.Y;
					const double ExistingMaximum = FrameAxis
						== EABTSM73BeamAFrameAxis::X
						? ExistingBounds.Max.X : ExistingBounds.Max.Y;
					if (!bSameSlab
						|| (!bCoversClampCell && !bOwnsFacadeJunction))
					{
						continue;
					}
					const double CoveredMinimum = FMath::Max(
						AvailableMinimum, ExistingMinimum);
					const double CoveredMaximum = FMath::Min(
						AvailableMaximum, ExistingMaximum);
					const double CoveredLength = CoveredMaximum - CoveredMinimum;
					if (CoveredLength > BestCoveredLength + GeometryToleranceCM)
					{
						BestCoveredLength = CoveredLength;
						MemberIndex = ExistingIndex;
						ResolvedFrameAxis = Existing.Axis;
						ResolvedRunMinimum = CoveredMinimum;
						ResolvedRunMaximum = CoveredMaximum;
					}
				}
				if (MemberIndex == INDEX_NONE)
				{
					for (int32 BlockingIndex = 0;
						BlockingIndex < Plan.Members.Num(); ++BlockingIndex)
					{
						const FPlannedMember& Blocking = Plan.Members[BlockingIndex];
						if (BoxesPenetrate(MinimumProbeBounds,
							PlannedMemberBounds(Blocking)))
						{
							UE_LOG(LogABTSRuntime, Warning,
								TEXT("[ABTS][M7.3-Beam-C3V3][Stage3FrameBlocker]")
								TEXT(" Anchor=%d Blocker=%d Axis=%d Kind=%d Stage=%d Course=%d Bounds=%s"),
								Band.AnchorBandId, BlockingIndex,
								static_cast<int32>(Blocking.Axis),
								static_cast<int32>(Blocking.SkeletonKind),
								static_cast<int32>(Blocking.ProducedStage),
								Blocking.CourseIndex,
								*PlannedMemberBounds(Blocking).ToString());
						}
					}
					UE_LOG(LogABTSRuntime, Warning,
						TEXT("[ABTS][M7.3-Beam-C3V3][Stage3AnchorWithoutFrame]")
						TEXT(" Anchor=%d Partition=%d Component=%d Face=%u Course=%d Normal=%.3f Tangent=%.3f Run=%.3f..%.3f"),
						Band.AnchorBandId, Band.FacadePartitionId,
						Partition->ComponentId, Band.FaceMask, FrameCourse,
						Band.FacadeCoordinateCM, Band.TangentCoordinateCM,
						RunMinimum, RunMaximum);
					++Plan.Summary.Stage3FacadeFitViolationCount;
					++Plan.Summary.Stage3AnchorBandWithoutFrameCount;
					continue;
				}
				bReusesExistingMember = true;
				}
			}

			const int32 FrameId = Plan.CommonExteriorFrames.Num();
			if (!bReusesExistingMember)
			{
				// Every collision/fallback branch converges here.  Preserve the lane
				// phase, but discard any fractional tail before a physical member is
				// admitted to the unitized plan.
				ResolvedRunMinimum = CellFaceAtOrAbove(ResolvedRunMinimum);
				ResolvedRunMaximum = CellFaceAtOrBelow(ResolvedRunMaximum);
				if (ResolvedRunMaximum - ResolvedRunMinimum
					< BlockUnitsCM - GeometryToleranceCM)
				{
					++Plan.Summary.Stage3FacadeFitViolationCount;
					++Plan.Summary.Stage3AnchorBandWithoutFrameCount;
				...
				const FVector ResolvedStart = bNormalX
					? Position(FrameNormalCoordinate, ResolvedRunMinimum, Z)
					: Position(ResolvedRunMinimum, FrameNormalCoordinate, Z);
				const FVector ResolvedEnd = bNormalX
					? Position(FrameNormalCoordinate, ResolvedRunMaximum, Z)
					: Position(ResolvedRunMaximum, FrameNormalCoordinate, Z);
				MemberIndex = Plan.Members.Num();
				if (!AddPlannedMember(Plan, EOwnerKind::BuildingGroupShell,
					ESkeletonMemberKind::FacadeCourse, FrameId, Partition->ComponentId,
					Span->SourceVolumeId, Band.OriginCoreCellId, FrameCourse,
					FMath::RoundToInt(ResolvedRunMinimum / BlockUnitsCM),
					FMath::RoundToInt(ResolvedRunMaximum / BlockUnitsCM), Band.FaceMask,
					FrameAxis, FrameAxis == EABTSM73BeamAFrameAxis::X
						? EABTSM73BeamAMemberRole::PrimaryBeam
						: EABTSM73BeamAMemberRole::SecondaryBeam,
					ResolvedStart, ResolvedEnd, OutError))
				{
					return false;
				}
				FPlannedMember& Member = Plan.Members[MemberIndex];
				Member.AnchorBandId = Band.AnchorBandId;
				Member.FacadePartitionId = Band.FacadePartitionId;
				Member.TargetFacadeSourceVolumeId = Span->SourceVolumeId;
				Member.ParentStage2MemberIndices = {
					Band.LowerMemberIndex, Band.UpperMemberIndex};
				Member.ExteriorFrameId = FrameId;
				Member.RequiredLowerMemberIndices.Add(Band.LowerMemberIndex);
				Member.RequiredInwardMemberIndices = Member.ParentStage2MemberIndices;
				Plan.Summary.TotalMemberLengthCM += FVector::Distance(
			...
			FCommonExteriorFramePlan& Frame =
				Plan.CommonExteriorFrames.AddDefaulted_GetRef();
			Frame.ExteriorFrameId = FrameId;
			Frame.FacadePartitionId = Band.FacadePartitionId;
			Frame.AnchorBandId = Band.AnchorBandId;
			Frame.ComponentId = Partition->ComponentId;
			Frame.CourseIndex = FrameCourse;
			Frame.FaceMask = Band.FaceMask;
			Frame.Axis = ResolvedFrameAxis;
			Frame.FacadeCoordinateCM = Band.FacadeCoordinateCM;
			Frame.TangentMinimumCM = ResolvedRunMinimum;
			Frame.TangentMaximumCM = ResolvedRunMaximum;
			Frame.LowerStage2MemberIndex = Band.LowerMemberIndex;
			Frame.UpperStage2MemberIndex = Band.UpperMemberIndex;
			Frame.MemberIndex = MemberIndex;
			Frame.bReusesExistingMember = bReusesExistingMember;
		}
		Plan.Summary.CommonExteriorFrameCount = Plan.CommonExteriorFrames.Num();
		for (const FCommonExteriorFramePlan& Frame : Plan.CommonExteriorFrames)
		{
			if (!Plan.Members.IsValidIndex(Frame.MemberIndex)
				|| !Plan.Members.IsValidIndex(Frame.LowerStage2MemberIndex)
				|| !Plan.Members.IsValidIndex(Frame.UpperStage2MemberIndex))
			{
				++Plan.Summary.Stage3ClampViolationCount;
				continue;
			}
			const FPlannedMember& Member = Plan.Members[Frame.MemberIndex];
			const FPlannedMember& Lower = Plan.Members[Frame.LowerStage2MemberIndex];
			const FPlannedMember& Upper = Plan.Members[Frame.UpperStage2MemberIndex];
			const FBox FrameBounds = PlannedMemberBounds(Member);
			const FBox LowerBounds = PlannedMemberBounds(Lower);
			const FBox UpperBounds = PlannedMemberBounds(Upper);
			const double RequiredClampOverlap = Frame.bReusesExistingMember
				? GeometryToleranceCM : BlockUnitsCM - GeometryToleranceCM;
			const bool bPerpendicular = (Frame.bReusesExistingMember
				|| Member.Axis != Lower.Axis)
				&& Member.Axis != EABTSM73BeamAFrameAxis::Z
				&& Lower.Axis == Upper.Axis;
			const bool bLowerClamp = FMath::Abs(
				LowerBounds.Max.Z - FrameBounds.Min.Z) <= GeometryToleranceCM
				&& SkeletonV3OverlapLength(LowerBounds.Min.X, LowerBounds.Max.X,
					FrameBounds.Min.X, FrameBounds.Max.X)
					>= RequiredClampOverlap
				&& SkeletonV3OverlapLength(LowerBounds.Min.Y, LowerBounds.Max.Y,
					FrameBounds.Min.Y, FrameBounds.Max.Y)
					>= RequiredClampOverlap;
			const bool bUpperClamp = FMath::Abs(
				FrameBounds.Max.Z - UpperBounds.Min.Z) <= GeometryToleranceCM
				&& SkeletonV3OverlapLength(FrameBounds.Min.X, FrameBounds.Max.X,
					UpperBounds.Min.X, UpperBounds.Max.X)
					>= RequiredClampOverlap
				&& SkeletonV3OverlapLength(FrameBounds.Min.Y, FrameBounds.Max.Y,
					UpperBounds.Min.Y, UpperBounds.Max.Y)
					>= RequiredClampOverlap;
			Plan.Summary.Stage3ClampViolationCount +=
				bPerpendicular && bLowerClamp && bUpperClamp ? 0 : 1;
		}
		Plan.Summary.Stage3FrameMilliseconds = ElapsedMilliseconds(FrameStartSeconds);

		// The ground sill consumes the real course-0 WFC contour, never a global
		// building AABB. X-running cells are emitted first; a perpendicular
		// Y-running side then reuses their grounded corner cell instead of creating
		// positive-volume overlap. The same cell-level substitution lets a sill
		// terminate on an already-grounded core rail without duplicating it.
		struct FGroundContourSpan
		{
		...
		TArray<FGroundContourSpan> GroundContourSpans;
		for (const FFacadePartitionPlan& Partition : Plan.FacadePartitions)
		{
			for (const FFacadePartitionCourseSpan& Span : Partition.CourseSpans)
			{
				if (Span.CourseIndex != 0)
				{
					continue;
				}
				FGroundContourSpan& GroundSpan =
					GroundContourSpans.AddDefaulted_GetRef();
				GroundSpan.FacadePartitionId = Partition.PartitionId;
				GroundSpan.ComponentId = Partition.ComponentId;
				GroundSpan.SourceVolumeId = Span.SourceVolumeId;
				GroundSpan.FaceMask = Partition.FaceMask;
				GroundSpan.Axis = Partition.FaceMask == NegativeX
					|| Partition.FaceMask == PositiveX
					? EABTSM73BeamAFrameAxis::Y
					: EABTSM73BeamAFrameAxis::X;
				GroundSpan.FacadeCoordinateCM = Partition.FacadeCoordinateCM;
				GroundSpan.TangentMinimumCM = Span.TangentMinimumCM;
				GroundSpan.TangentMaximumCM = Span.TangentMaximumCM;
			}
		}
		GroundContourSpans.Sort([](const FGroundContourSpan& A,
			const FGroundContourSpan& B)
		{
			if (A.ComponentId != B.ComponentId) return A.ComponentId < B.ComponentId;
			// X owns every same-level corner. Y observes that grounded cell and
			// records a deterministic reuse instead of penetrating it.
			if (A.Axis != B.Axis)
			{
				return A.Axis == EABTSM73BeamAFrameAxis::X;
			}
			if (A.FaceMask != B.FaceMask) return A.FaceMask < B.FaceMask;
			if (!FMath::IsNearlyEqual(A.FacadeCoordinateCM, B.FacadeCoordinateCM,
				GeometryToleranceCM))
		...
		TMap<int32, int32> GroundLoopByComponent;
		TSet<FString> ConsumedGroundCells;
		auto GroundLoopFor = [&GroundLoopByComponent](const int32 ComponentId)
		{
			if (const int32* Existing = GroundLoopByComponent.Find(ComponentId))
			{
				return *Existing;
			}
			const int32 LoopId = GroundLoopByComponent.Num();
			GroundLoopByComponent.Add(ComponentId, LoopId);
			return LoopId;
		};
		auto AddGroundSillLedger = [&Plan](const FGroundContourSpan& Span,
			const int32 LoopId, const double Minimum, const double Maximum,
			const int32 MemberIndex, const bool bReused)
		{
			FGroundSillSegmentPlan& Segment =
				Plan.GroundSillSegments.AddDefaulted_GetRef();
			Segment.GroundSillSegmentId = Plan.GroundSillSegments.Num() - 1;
			Segment.GroundSillLoopId = LoopId;
			Segment.FacadePartitionId = Span.FacadePartitionId;
			Segment.ComponentId = Span.ComponentId;
			Segment.FaceMask = Span.FaceMask;
			Segment.Axis = Span.Axis;
			Segment.FacadeCoordinateCM = Span.FacadeCoordinateCM;
			Segment.TangentMinimumCM = Minimum;
			Segment.TangentMaximumCM = Maximum;
			Segment.MemberIndex = MemberIndex;
			Segment.bReusesGroundedCoreMember = bReused;
			if (!bReused && Plan.Members.IsValidIndex(MemberIndex)
				&& Plan.Members[MemberIndex].ProducedStage
					== EABTSM73BeamC3GenerationStage::CommonExteriorFrame)
			{
				Plan.Members[MemberIndex].GroundSillSegmentId =
					Segment.GroundSillSegmentId;
			}
		};
		for (const FGroundContourSpan& Span : GroundContourSpans)
		{
			const int32 LoopId = GroundLoopFor(Span.ComponentId);
			const double Minimum = CellFaceAtOrAbove(Span.TangentMinimumCM);
			const double Maximum = CellFaceAtOrBelow(Span.TangentMaximumCM);
			if (Maximum - Minimum < BlockUnitsCM - GeometryToleranceCM)
			{
				++Plan.Summary.Stage3FacadeFitViolationCount;
				continue;
			}
			const bool bNormalX = Span.FaceMask == NegativeX
				|| Span.FaceMask == PositiveX;
			const bool bPositiveFace = Span.FaceMask == PositiveX
				|| Span.FaceMask == PositiveY;
			const double NormalCoordinate = Span.FacadeCoordinateCM
				+ (bPositiveFace ? -1.0 : 1.0) * BlockUnitsCM * 0.5;
			const double GroundZ = Plan.Components.IsValidIndex(Span.ComponentId)
				? Plan.Components[Span.ComponentId].GroundPlaneZCM : 0.0;
			const double Z = GroundZ + BlockUnitsCM * 0.5;
			double FreeMinimum = 0.0;
			double FreeMaximum = 0.0;
			bool bHasFreeRun = false;
			auto FlushFreeRun = [&]() -> bool
			{
				if (!bHasFreeRun)
				{
					return true;
				}
				double Cursor = FreeMinimum;
				while (Cursor < FreeMaximum - GeometryToleranceCM)
				{
					const double End = FMath::Min(
						Cursor + MaximumHorizontalUnits * static_cast<double>(BlockUnitsCM),
						FreeMaximum);
					const FVector Start = bNormalX
						? Position(NormalCoordinate, Cursor, Z)
						: Position(Cursor, NormalCoordinate, Z);
					const FVector Finish = bNormalX
						? Position(NormalCoordinate, End, Z)
						: Position(End, NormalCoordinate, Z);
					const int32 SillId = Plan.GroundSillSegments.Num();
					const int32 MemberIndex = Plan.Members.Num();
					if (!AddPlannedMember(Plan, EOwnerKind::BuildingGroupShell,
						ESkeletonMemberKind::GroundSillCourse, SillId,
						Span.ComponentId, Span.SourceVolumeId, INDEX_NONE, 0,
						FMath::RoundToInt(Cursor / BlockUnitsCM),
						FMath::RoundToInt(End / BlockUnitsCM), Span.FaceMask,
						Span.Axis, Span.Axis == EABTSM73BeamAFrameAxis::X
							? EABTSM73BeamAMemberRole::PrimaryBeam
							: EABTSM73BeamAMemberRole::SecondaryBeam,
						Start, Finish, OutError))
					{
						return false;
					}
					FPlannedMember& Member = Plan.Members[MemberIndex];
					Member.FacadePartitionId = Span.FacadePartitionId;
					Member.TargetFacadeSourceVolumeId = Span.SourceVolumeId;
					Member.bRequiresGroundSeat = true;
					AddGroundSillLedger(Span, LoopId, Cursor, End, MemberIndex, false);
					++Plan.Summary.EmittedGroundSillSegmentCount;
					Plan.Summary.TotalMemberLengthCM += FVector::Distance(Start, Finish);
					Cursor = End;
				}
				bHasFreeRun = false;
				return true;
			};
			for (double CellMinimum = Minimum;
				CellMinimum < Maximum - GeometryToleranceCM;
				CellMinimum += BlockUnitsCM)
			{
				const double CellMaximum = FMath::Min(
					CellMinimum + BlockUnitsCM, Maximum);
				const FString CellKey = FString::Printf(TEXT("%d:%u:%lld:%lld"),
					Span.ComponentId, Span.FaceMask, QHash(Span.FacadeCoordinateCM),
					QHash(CellMinimum));
				if (ConsumedGroundCells.Contains(CellKey))
				{
					continue;
				}
				ConsumedGroundCells.Add(CellKey);
				const FVector CellStart = bNormalX
					? Position(NormalCoordinate, CellMinimum, Z)
					: Position(CellMinimum, NormalCoordinate, Z);
				const FVector CellEnd = bNormalX
					? Position(NormalCoordinate, CellMaximum, Z)
					: Position(CellMaximum, NormalCoordinate, Z);
				FPlannedMember CellProbe;
				CellProbe.Axis = Span.Axis;
				CellProbe.LocalStart = CellStart;
				CellProbe.LocalEnd = CellEnd;
				const FBox CellBounds = PlannedMemberBounds(CellProbe);
				int32 CoveringGroundMember = INDEX_NONE;
				bool bBlocked = false;
				for (int32 ExistingIndex = 0; ExistingIndex < Plan.Members.Num(); ++ExistingIndex)
				{
					const FPlannedMember& Existing = Plan.Members[ExistingIndex];
					const FBox ExistingBounds = PlannedMemberBounds(Existing);
					if (!BoxesPenetrate(CellBounds, ExistingBounds))
					{
						continue;
					}
					const bool bCoversCell = Existing.bRequiresGroundSeat
						&& Existing.Axis != EABTSM73BeamAFrameAxis::Z
						&& ExistingBounds.Min.X <= CellBounds.Min.X + GeometryToleranceCM
						&& ExistingBounds.Max.X >= CellBounds.Max.X - GeometryToleranceCM
						&& ExistingBounds.Min.Y <= CellBounds.Min.Y + GeometryToleranceCM
						&& ExistingBounds.Max.Y >= CellBounds.Max.Y - GeometryToleranceCM
						&& FMath::Abs(ExistingBounds.Min.Z - CellBounds.Min.Z)
							<= GeometryToleranceCM
						&& FMath::Abs(ExistingBounds.Max.Z - CellBounds.Max.Z)
							<= GeometryToleranceCM;
					if (bCoversCell)
					{
						CoveringGroundMember = ExistingIndex;
						break;
					}
					bBlocked = true;
				}
				if (CoveringGroundMember != INDEX_NONE || bBlocked)
				{
					if (!FlushFreeRun()) return false;
					if (CoveringGroundMember != INDEX_NONE)
					{
						AddGroundSillLedger(Span, LoopId, CellMinimum, CellMaximum,
							CoveringGroundMember, true);
						++Plan.Summary.ReusedGroundSillSegmentCount;
					}
					else
					{
						++Plan.Summary.GroundSillConflictOmissionCount;
					}
					continue;
				}
				if (!bHasFreeRun)
				{
					FreeMinimum = CellMinimum;
					bHasFreeRun = true;
				}
				FreeMaximum = CellMaximum;
			}
			if (!FlushFreeRun()) return false;
		}
		Plan.Summary.GroundSillLoopCount = GroundLoopByComponent.Num();
		Plan.Summary.GroundSillSegmentCount = Plan.GroundSillSegments.Num();

		const double ColumnStartSeconds = FPlatformTime::Seconds();
		const int32 Stage3HorizontalMemberCount = Plan.Members.Num();
		auto EnvelopeContainsColumnAt = [&Plan](const int32 ComponentId,
			const uint8 FaceMask, const double FacadeCoordinateCM,
			const int32 Course, const double Tangent)
		{
			const double GroundZ = Plan.Components.IsValidIndex(ComponentId)
				? Plan.Components[ComponentId].GroundPlaneZCM : 0.0;
			const bool bPositiveFace = FaceMask == PositiveX || FaceMask == PositiveY;
			const bool bNormalX = FaceMask == NegativeX || FaceMask == PositiveX;
			const double Normal = FacadeCoordinateCM
				+ (bPositiveFace ? -1.0 : 1.0) * BlockUnitsCM * 0.5;
			const FVector Point = bNormalX
				? Position(Normal, Tangent, GroundZ + (Course + 0.5) * BlockUnitsCM)
				: Position(Tangent, Normal, GroundZ + (Course + 0.5) * BlockUnitsCM);
			for (const FResolvedFacadeEnvelopeVolume& Volume
				: Plan.ResolvedFacadeEnvelopeVolumes)
			{
				if (Volume.ComponentId == ComponentId
					&& Volume.LocalBounds.ExpandBy(GeometryToleranceCM).IsInsideOrOn(Point))
				{
					return true;
				}
			}
		...
		TMap<FString, TArray<const FCommonExteriorFramePlan*>> FramesByPlane;
		for (const FCommonExteriorFramePlan& Frame : Plan.CommonExteriorFrames)
		{
			const FString PlaneKey = FString::Printf(TEXT("%d:%u:%lld"),
				Frame.ComponentId, Frame.FaceMask, QHash(Frame.FacadeCoordinateCM));
			FramesByPlane.FindOrAdd(PlaneKey).Add(&Frame);
		}
		TSet<int32> DownwardConnectedFrameIds;
		TSet<FString> EmittedColumnKeys;
		TArray<FString> PlaneKeys;
		FramesByPlane.GetKeys(PlaneKeys);
		PlaneKeys.Sort();
		for (const FString& PlaneKey : PlaneKeys)
		{
			TArray<const FCommonExteriorFramePlan*>& Frames = FramesByPlane[PlaneKey];
			Frames.Sort([](const FCommonExteriorFramePlan& A,
				const FCommonExteriorFramePlan& B)
			{
				return A.CourseIndex != B.CourseIndex
					? A.CourseIndex < B.CourseIndex
					: A.ExteriorFrameId < B.ExteriorFrameId;
			});
			if (Frames.IsEmpty())
			{
				continue;
			}
			for (const FCommonExteriorFramePlan* UpperFramePtr : Frames)
			{
				const FCommonExteriorFramePlan& UpperFrame = *UpperFramePtr;
				const FCommonExteriorFramePlan* LowerFrame = nullptr;
				double BestOverlap = -1.0;
				for (const FCommonExteriorFramePlan* Candidate : Frames)
				{
					if (Candidate->CourseIndex >= UpperFrame.CourseIndex)
					{
						continue;
					}
					const double Overlap = FMath::Min(Candidate->TangentMaximumCM,
						UpperFrame.TangentMaximumCM) - FMath::Max(
						Candidate->TangentMinimumCM, UpperFrame.TangentMinimumCM);
					if (Overlap < BlockUnitsCM - GeometryToleranceCM)
					{
						continue;
					}
					if (LowerFrame == nullptr
						|| Candidate->CourseIndex > LowerFrame->CourseIndex
						|| (Candidate->CourseIndex == LowerFrame->CourseIndex
							&& Overlap > BestOverlap + GeometryToleranceCM))
					{
						LowerFrame = Candidate;
						BestOverlap = Overlap;
					}
				}

				const FGroundSillSegmentPlan* GroundSill = nullptr;
				double TangentMinimum = UpperFrame.TangentMinimumCM;
				double TangentMaximum = UpperFrame.TangentMaximumCM;
				if (LowerFrame != nullptr)
				{
					TangentMinimum = FMath::Max(TangentMinimum,
						LowerFrame->TangentMinimumCM);
					TangentMaximum = FMath::Min(TangentMaximum,
						LowerFrame->TangentMaximumCM);
				}
				else
				{
					double BestSillOverlap = -1.0;
					for (const FGroundSillSegmentPlan& Sill : Plan.GroundSillSegments)
					{
						if (Sill.ComponentId != UpperFrame.ComponentId
							|| Sill.FaceMask != UpperFrame.FaceMask
							|| !FMath::IsNearlyEqual(Sill.FacadeCoordinateCM,
								UpperFrame.FacadeCoordinateCM, GeometryToleranceCM))
						{
							continue;
						}
						const double Overlap = FMath::Min(Sill.TangentMaximumCM,
							UpperFrame.TangentMaximumCM) - FMath::Max(
							Sill.TangentMinimumCM, UpperFrame.TangentMinimumCM);
						if (Overlap >= BlockUnitsCM - GeometryToleranceCM
							&& Overlap > BestSillOverlap + GeometryToleranceCM)
						{
							GroundSill = &Sill;
							BestSillOverlap = Overlap;
						}
					}
					if (GroundSill != nullptr)
					{
						TangentMinimum = FMath::Max(TangentMinimum,
							GroundSill->TangentMinimumCM);
						TangentMaximum = FMath::Min(TangentMaximum,
							GroundSill->TangentMaximumCM);
					}
				}
				if (LowerFrame == nullptr && GroundSill == nullptr)
				{
					continue;
				}
				TArray<double> CandidateStations;
				CandidateStations.Add(TangentMinimum + BlockUnitsCM * 0.5);
				if (TangentMaximum - TangentMinimum
					>= 2.0 * BlockUnitsCM - GeometryToleranceCM)
				{
					CandidateStations.Add(TangentMaximum - BlockUnitsCM * 0.5);
				}
				for (const double Station : CandidateStations)
				{
					bool bContinuousFacade = true;
					const int32 LowerCourse = LowerFrame != nullptr
						? LowerFrame->CourseIndex : 0;
					for (int32 Course = LowerCourse;
						Course <= UpperFrame.CourseIndex; ++Course)
					{
						bContinuousFacade &= EnvelopeContainsColumnAt(UpperFrame.ComponentId,
							UpperFrame.FaceMask, UpperFrame.FacadeCoordinateCM,
							Course, Station);
						if (!bContinuousFacade) break;
					}
					if (!bContinuousFacade)
					{
						++Plan.Summary.Stage3FacadeFitViolationCount;
						continue;
					}
					const double GroundZ = Plan.Components.IsValidIndex(UpperFrame.ComponentId)
						? Plan.Components[UpperFrame.ComponentId].GroundPlaneZCM : 0.0;
					const double StartZ = LowerFrame != nullptr
						? GroundZ + (LowerFrame->CourseIndex + 1) * BlockUnitsCM
						: GroundZ + BlockUnitsCM;
					const double EndZ = GroundZ
						+ UpperFrame.CourseIndex * BlockUnitsCM;
					if (EndZ - StartZ < BlockUnitsCM - GeometryToleranceCM)
					{
						DownwardConnectedFrameIds.Add(UpperFrame.ExteriorFrameId);
						continue;
					}
					const FString ColumnKey = FString::Printf(TEXT("%s:%d:%d:%lld"),
						*PlaneKey, LowerFrame != nullptr
							? LowerFrame->ExteriorFrameId : INDEX_NONE,
						UpperFrame.ExteriorFrameId, QHash(Station));
					if (EmittedColumnKeys.Contains(ColumnKey))
					{
						DownwardConnectedFrameIds.Add(UpperFrame.ExteriorFrameId);
						continue;
					}
					const int32 ColumnId = Plan.ExteriorColumns.Num();
					const bool bPositiveFace = UpperFrame.FaceMask == PositiveX
						|| UpperFrame.FaceMask == PositiveY;
					const double ColumnNormalCoordinate = UpperFrame.FacadeCoordinateCM
						+ (bPositiveFace ? -1.0 : 1.0) * BlockUnitsCM * 0.5;
					FExteriorColumnPlan Column;
					Column.ExteriorColumnId = ColumnId;
					Column.FacadePartitionId = UpperFrame.FacadePartitionId;
					Column.ComponentId = UpperFrame.ComponentId;
					Column.FaceMask = UpperFrame.FaceMask;
					Column.FacadeCoordinateCM = UpperFrame.FacadeCoordinateCM;
					Column.TangentCoordinateCM = Station;
					Column.LowerExteriorFrameId = LowerFrame != nullptr
						? LowerFrame->ExteriorFrameId : INDEX_NONE;
					Column.UpperExteriorFrameId = UpperFrame.ExteriorFrameId;
					Column.bGroundToFirstFrame = LowerFrame == nullptr;
					Column.GroundSillSegmentId = GroundSill != nullptr
						? GroundSill->GroundSillSegmentId : INDEX_NONE;
					const int32 ColumnMemberStart = Plan.Members.Num();
					const double ColumnLengthStart = Plan.Summary.TotalMemberLengthCM;
					double SegmentStartZ = StartZ;
					int32 LowerMemberIndex = LowerFrame != nullptr
						? LowerFrame->MemberIndex : GroundSill->MemberIndex;
					bool bColumnRejected = false;
					while (SegmentStartZ < EndZ - GeometryToleranceCM)
					{
						const double SegmentEndZ = FMath::Min(
							EndZ, SegmentStartZ + 720.0);
						const bool bNormalX = UpperFrame.FaceMask == NegativeX
							|| UpperFrame.FaceMask == PositiveX;
						const FVector Start = bNormalX
							? Position(ColumnNormalCoordinate, Station, SegmentStartZ)
							: Position(Station, ColumnNormalCoordinate, SegmentStartZ);
						const FVector End = Start + FVector(0.0, 0.0,
							SegmentEndZ - SegmentStartZ);
						FPlannedMember Probe;
						Probe.Axis = EABTSM73BeamAFrameAxis::Z;
						Probe.LocalStart = Start;
						Probe.LocalEnd = End;
						if (PenetratesExisting(Probe))
						{
							bColumnRejected = true;
							break;
						}
						const int32 MemberIndex = Plan.Members.Num();
						if (!AddPlannedMember(Plan, EOwnerKind::BuildingGroupShell,
							LowerFrame != nullptr ? ESkeletonMemberKind::ExteriorPost
								: ESkeletonMemberKind::GroundExteriorPost,
							ColumnId, UpperFrame.ComponentId, INDEX_NONE, INDEX_NONE,
							FMath::FloorToInt((SegmentStartZ - GroundZ) / BlockUnitsCM),
							FMath::RoundToInt(Station / BlockUnitsCM), INDEX_NONE,
							UpperFrame.FaceMask, EABTSM73BeamAFrameAxis::Z,
							EABTSM73BeamAMemberRole::Post, Start, End, OutError))
						{
							return false;
						}
						FPlannedMember& Member = Plan.Members[MemberIndex];
						Member.FacadePartitionId = UpperFrame.FacadePartitionId;
						if (LowerFrame != nullptr)
						{
							Member.ParentStage2MemberIndices = {
								LowerFrame->LowerStage2MemberIndex,
								LowerFrame->UpperStage2MemberIndex,
								UpperFrame.LowerStage2MemberIndex,
								UpperFrame.UpperStage2MemberIndex};
						}
						else
						{
							Member.ParentStage2MemberIndices = {
						...
						Member.ExteriorColumnId = ColumnId;
						Member.LowerExteriorFrameId = LowerFrame != nullptr
							? LowerFrame->ExteriorFrameId : INDEX_NONE;
						Member.UpperExteriorFrameId = UpperFrame.ExteriorFrameId;
						Member.GroundSillSegmentId = GroundSill != nullptr
							? GroundSill->GroundSillSegmentId : INDEX_NONE;
						Member.RequiredLowerMemberIndices.Add(LowerMemberIndex);
						Member.RequiredInwardMemberIndices.Add(UpperFrame.MemberIndex);
						Member.RequiredInwardMemberIndices.Add(LowerMemberIndex);
						Column.SegmentMemberIndices.Add(MemberIndex);
						LowerMemberIndex = MemberIndex;
						SegmentStartZ = SegmentEndZ;
						Plan.Summary.TotalMemberLengthCM += FVector::Distance(Start, End);
					}
					if (bColumnRejected)
					{
						Plan.Members.SetNum(ColumnMemberStart, EAllowShrinking::No);
						Plan.Summary.TotalMemberLengthCM = ColumnLengthStart;
						if (LowerFrame == nullptr)
						{
							++Plan.Summary.GroundExteriorColumnConflictOmissionCount;
						}
						else
						{
							++Plan.Summary.Stage3ColumnFrameViolationCount;
						}
						continue;
					}
					...
					DownwardConnectedFrameIds.Add(UpperFrame.ExteriorFrameId);
					if (LowerFrame != nullptr
						&& LowerFrame->FacadePartitionId != UpperFrame.FacadePartitionId)
					{
						++Plan.Summary.Stage3CrossPartitionColumnCount;
					}
					Plan.Summary.ExteriorColumnSegmentCount +=
						Column.SegmentMemberIndices.Num();
					if (LowerFrame == nullptr)
					{
					...
					Plan.ExteriorColumns.Add(MoveTemp(Column));
				}
			}
		}
		// A setback can move an upper facade inward relative to the ground sill,
		// and neighbouring cores can author overlapping frames under different
		// partitions. For every frame still lacking a post, resolve a geometric
		// downward seat at its actual XY station. This intentionally ignores core
		// ownership: the facade is one building-wide support network.
		for (const FCommonExteriorFramePlan& UpperFrame : Plan.CommonExteriorFrames)
		{
			if (DownwardConnectedFrameIds.Contains(UpperFrame.ExteriorFrameId)
				|| !Plan.Members.IsValidIndex(UpperFrame.MemberIndex))
			{
				continue;
			}
			const FBox UpperBounds = PlannedMemberBounds(
				Plan.Members[UpperFrame.MemberIndex]);
			const bool bNormalX = UpperFrame.FaceMask == NegativeX
				|| UpperFrame.FaceMask == PositiveX;
			const bool bPositiveFace = UpperFrame.FaceMask == PositiveX
				|| UpperFrame.FaceMask == PositiveY;
			const double Normal = UpperFrame.FacadeCoordinateCM
				+ (bPositiveFace ? -1.0 : 1.0) * BlockUnitsCM * 0.5;
			TArray<double> Stations;
			for (double Station = CellFaceAtOrAbove(UpperFrame.TangentMinimumCM)
					+ BlockUnitsCM * 0.5;
				Station <= UpperFrame.TangentMaximumCM
					- BlockUnitsCM * 0.5 + GeometryToleranceCM;
				Station += BlockUnitsCM)
			{
				Stations.Add(Station);
			}
			if (Stations.IsEmpty())
			{
				Stations.Add((UpperFrame.TangentMinimumCM
					+ UpperFrame.TangentMaximumCM) * 0.5);
			}
			int32 SuccessfulStationCount = 0;
			for (const double Station : Stations)
			{
				const double X = bNormalX ? Normal : Station;
				const double Y = bNormalX ? Station : Normal;
				int32 LowerSupportMemberIndex = INDEX_NONE;
				double BestSupportTopZ = -DBL_MAX;
				for (int32 MemberIndex = 0;
					MemberIndex < Stage3HorizontalMemberCount; ++MemberIndex)
				{
					const FPlannedMember& Candidate = Plan.Members[MemberIndex];
					if (MemberIndex == UpperFrame.MemberIndex
						|| Candidate.Axis == EABTSM73BeamAFrameAxis::Z)
					{
						continue;
					}
					const FBox Bounds = PlannedMemberBounds(Candidate);
					const bool bCoversStation = Bounds.Min.X
						<= X - BlockUnitsCM * 0.5 + GeometryToleranceCM
						&& Bounds.Max.X >= X + BlockUnitsCM * 0.5 - GeometryToleranceCM
						&& Bounds.Min.Y <= Y - BlockUnitsCM * 0.5 + GeometryToleranceCM
						&& Bounds.Max.Y >= Y + BlockUnitsCM * 0.5 - GeometryToleranceCM;
					if (bCoversStation
						&& Bounds.Max.Z <= UpperBounds.Min.Z + GeometryToleranceCM
						&& Bounds.Max.Z > BestSupportTopZ + GeometryToleranceCM)
					{
						LowerSupportMemberIndex = MemberIndex;
						BestSupportTopZ = Bounds.Max.Z;
					}
				}
				const double GroundZ = Plan.Components.IsValidIndex(UpperFrame.ComponentId)
					? Plan.Components[UpperFrame.ComponentId].GroundPlaneZCM : 0.0;
				const double StartZ = LowerSupportMemberIndex != INDEX_NONE
					? BestSupportTopZ : GroundZ;
				const double EndZ = UpperBounds.Min.Z;
				if (EndZ - StartZ < BlockUnitsCM - GeometryToleranceCM)
				{
					DownwardConnectedFrameIds.Add(UpperFrame.ExteriorFrameId);
					break;
				}
				FPlannedMember WholeProbe;
				WholeProbe.Axis = EABTSM73BeamAFrameAxis::Z;
				WholeProbe.LocalStart = Position(X, Y, StartZ);
				WholeProbe.LocalEnd = Position(X, Y, EndZ);
				if (PenetratesExisting(WholeProbe))
				{
					continue;
				}
				const int32 ColumnId = Plan.ExteriorColumns.Num();
				...
				Column.TangentCoordinateCM = Station;
				Column.UpperExteriorFrameId = UpperFrame.ExteriorFrameId;
				Column.bGroundToFirstFrame = LowerSupportMemberIndex == INDEX_NONE;
				if (LowerSupportMemberIndex != INDEX_NONE)
				{
					if (const FCommonExteriorFramePlan* LowerFrame =
						Plan.CommonExteriorFrames.FindByPredicate(
							[LowerSupportMemberIndex](const FCommonExteriorFramePlan& Frame)
							{
								return Frame.MemberIndex == LowerSupportMemberIndex;
							}))
					{
						Column.LowerExteriorFrameId = LowerFrame->ExteriorFrameId;
						Plan.Summary.Stage3CrossPartitionColumnCount +=
							LowerFrame->FacadePartitionId != UpperFrame.FacadePartitionId ? 1 : 0;
					}
					if (const FGroundSillSegmentPlan* Sill =
						Plan.GroundSillSegments.FindByPredicate(
							[LowerSupportMemberIndex](const FGroundSillSegmentPlan& Segment)
							{
								return Segment.MemberIndex == LowerSupportMemberIndex;
							}))
					{
						Column.GroundSillSegmentId = Sill->GroundSillSegmentId;
						Column.bGroundToFirstFrame = true;
					}
				}
				double SegmentStartZ = StartZ;
				int32 LowerMemberIndex = LowerSupportMemberIndex;
				while (SegmentStartZ < EndZ - GeometryToleranceCM)
				{
					const double SegmentEndZ = FMath::Min(EndZ, SegmentStartZ + 720.0);
					const int32 MemberIndex = Plan.Members.Num();
					if (!AddPlannedMember(Plan, EOwnerKind::BuildingGroupShell,
						Column.bGroundToFirstFrame
							? ESkeletonMemberKind::GroundExteriorPost
						...
						FMath::RoundToInt(Station / BlockUnitsCM), INDEX_NONE,
						UpperFrame.FaceMask, EABTSM73BeamAFrameAxis::Z,
						EABTSM73BeamAMemberRole::Post, Position(X, Y, SegmentStartZ),
						Position(X, Y, SegmentEndZ), OutError))
					{
						return false;
					}
					FPlannedMember& Member = Plan.Members[MemberIndex];
					...
					Member.ParentStage2MemberIndices = {
						...
						UpperFrame.UpperStage2MemberIndex};
					Member.ExteriorColumnId = ColumnId;
					Member.LowerExteriorFrameId = Column.LowerExteriorFrameId;
					Member.UpperExteriorFrameId = UpperFrame.ExteriorFrameId;
					Member.GroundSillSegmentId = Column.GroundSillSegmentId;
					if (LowerMemberIndex != INDEX_NONE)
					{
						Member.RequiredLowerMemberIndices.Add(LowerMemberIndex);
					}
					else
					...
					Member.RequiredInwardMemberIndices.Add(UpperFrame.MemberIndex);
					Column.SegmentMemberIndices.Add(MemberIndex);
					Plan.Summary.TotalMemberLengthCM += SegmentEndZ - SegmentStartZ;
					LowerMemberIndex = MemberIndex;
					SegmentStartZ = SegmentEndZ;
				}
				Plan.Summary.ExteriorColumnSegmentCount += Column.SegmentMemberIndices.Num();
				if (Column.bGroundToFirstFrame)
				{
					++Plan.Summary.GroundExteriorColumnCount;
					Plan.Summary.GroundExteriorColumnSegmentCount +=
						Column.SegmentMemberIndices.Num();
				...
				Plan.ExteriorColumns.Add(MoveTemp(Column));
				DownwardConnectedFrameIds.Add(UpperFrame.ExteriorFrameId);
				if (++SuccessfulStationCount >= 2)
				{
					break;
				}
			}
		}
		// Several anchor bands can intentionally share one physical horizontal
		// member. A post supporting that member closes every logical ledger row.
		for (const FCommonExteriorFramePlan& Connected : Plan.CommonExteriorFrames)
		{
			if (Connected.bReusesExistingMember
				&& Plan.Members.IsValidIndex(Connected.MemberIndex)
				&& (Plan.Members[Connected.MemberIndex].bRequiresGroundSeat
					|| !Plan.Members[Connected.MemberIndex]
						.RequiredLowerMemberIndices.IsEmpty()))
			{
				DownwardConnectedFrameIds.Add(Connected.ExteriorFrameId);
			}
			if (!DownwardConnectedFrameIds.Contains(Connected.ExteriorFrameId))
			{
				continue;
			}
			for (const FCommonExteriorFramePlan& Alias : Plan.CommonExteriorFrames)
			{
				if (Alias.MemberIndex == Connected.MemberIndex)
				{
					DownwardConnectedFrameIds.Add(Alias.ExteriorFrameId);
				}
			}
		}
		// A logical corner frame may reuse a perpendicular physical frame. It is
		// closed when that same-course junction overlaps a frame that already owns
		// a downward post, even when the two ledger rows reference different bricks.
		...
		while (bPropagated)
		{
			...
			for (const FCommonExteriorFramePlan& Alias : Plan.CommonExteriorFrames)
			{
				if (DownwardConnectedFrameIds.Contains(Alias.ExteriorFrameId)
					|| !Plan.Members.IsValidIndex(Alias.MemberIndex))
				{
					continue;
				}
				const FBox AliasBounds = PlannedMemberBounds(Plan.Members[Alias.MemberIndex]);
				for (const FCommonExteriorFramePlan& Connected : Plan.CommonExteriorFrames)
				{
					if (Connected.CourseIndex == Alias.CourseIndex
						&& Connected.ComponentId == Alias.ComponentId
						&& DownwardConnectedFrameIds.Contains(Connected.ExteriorFrameId)
						&& Plan.Members.IsValidIndex(Connected.MemberIndex)
						&& BoxesPenetrate(AliasBounds,
							PlannedMemberBounds(Plan.Members[Connected.MemberIndex])))
					{
						DownwardConnectedFrameIds.Add(Alias.ExteriorFrameId);
						bPropagated = true;
						break;
		...
		Plan.Summary.ExteriorColumnCount = Plan.ExteriorColumns.Num();
		for (const FCommonExteriorFramePlan& Frame : Plan.CommonExteriorFrames)
		{
			if (!DownwardConnectedFrameIds.Contains(Frame.ExteriorFrameId))
			{
				++Plan.Summary.Stage3FrameDownwardConnectionViolationCount;
				UE_LOG(LogABTSRuntime, Warning,
					TEXT("[ABTS][M7.3-Beam-C3V3][Stage3FrameWithoutDownwardPath]")
					TEXT(" Frame=%d Partition=%d Anchor=%d Component=%d Face=%u Course=%d Normal=%.3f Tangent=%.3f..%.3f Reused=%d Member=%d"),
					Frame.ExteriorFrameId, Frame.FacadePartitionId,
					Frame.AnchorBandId, Frame.ComponentId, Frame.FaceMask,
					Frame.CourseIndex, Frame.FacadeCoordinateCM,
					Frame.TangentMinimumCM, Frame.TangentMaximumCM,
					Frame.bReusesExistingMember ? 1 : 0, Frame.MemberIndex);
			}
		}
		Plan.Summary.Stage3ColumnMilliseconds = ElapsedMilliseconds(ColumnStartSeconds);

		if (Plan.Summary.CommonExteriorFrameCount <= 0
		...
		{
			OutError = FString::Printf(
				TEXT("BeamC3V3Stage3ContractFailed:Frames=%d/%d:FrameEmitted=%d:FrameReused=%d:AnchorWithoutFrame=%d:FrameWithoutDownward=%d:CrossPartitionColumns=%d:SillLoops=%d:SillSegments=%d:SillEmitted=%d:SillReused=%d:SillConflictOmissions=%d:Columns=%d:GroundColumns=%d:Segments=%d:GroundSegments=%d:GroundColumnConflictOmissions=%d:ParentViolations=%d:ClampViolations=%d:ColumnFrameViolations=%d:FacadeFitViolations=%d"),
				Plan.Summary.CommonExteriorFrameCount,
				Plan.Summary.FacadeHeightAnchorBandCount,
				Plan.Summary.EmittedExteriorFrameCount,
				Plan.Summary.ReusedExteriorFrameCount,
				...
				Plan.Summary.Stage3FrameDownwardConnectionViolationCount,
				Plan.Summary.Stage3CrossPartitionColumnCount,
				Plan.Summary.GroundSillLoopCount,
		...
		const double DAGStartSeconds = FPlatformTime::Seconds();
		Plan.Summary.PenetrationCount = 0;
		TArray<bool> GroundedMembers;
		if (!ValidateNoPenetration(Plan, OutError)
			|| !ResolveGroundedMemberMask(Plan, GroundedMembers, OutError)
			|| !ValidatePreflightCaps(Profile.BeamSettings.BeamB.BeamA, Plan, OutError))
		{
			return false;
		}
		Plan.Summary.Stage3StaticDAGMilliseconds = ElapsedMilliseconds(DAGStartSeconds);
		Plan.Summary.PlannedMemberCount = Plan.Members.Num();
		Plan.Summary.PlannedSeatCount += Plan.Members.Num() - Stage2MemberCount;
		FString Canonical = FString::Printf(TEXT("Stage2=%lld"), Stage2GeometryHash);
		for (const FCommonExteriorFramePlan& Frame : Plan.CommonExteriorFrames)
		{
			Canonical += FString::Printf(
				TEXT("|EF:%d:F%d:B%d:C%d:Q%d:M%u:A%d:N%lld:TA%lld:TB%lld:R%d"),
				Frame.ExteriorFrameId, Frame.FacadePartitionId, Frame.AnchorBandId,
				Frame.ComponentId, Frame.CourseIndex, Frame.FaceMask,
				static_cast<int32>(Frame.Axis), QHash(Frame.FacadeCoordinateCM),
				QHash(Frame.TangentMinimumCM), QHash(Frame.TangentMaximumCM),
				Frame.bReusesExistingMember ? Frame.MemberIndex
					: -Frame.MemberIndex - 1);
		}
		for (const FGroundSillSegmentPlan& Sill : Plan.GroundSillSegments)
		{
			Canonical += FString::Printf(
				TEXT("|GS:%d:L%d:F%d:C%d:M%u:A%d:N%lld:TA%lld:TB%lld:R%d"),
				Sill.GroundSillSegmentId, Sill.GroundSillLoopId,
				Sill.FacadePartitionId, Sill.ComponentId, Sill.FaceMask,
				static_cast<int32>(Sill.Axis), QHash(Sill.FacadeCoordinateCM),
				QHash(Sill.TangentMinimumCM), QHash(Sill.TangentMaximumCM),
				Sill.bReusesGroundedCoreMember ? Sill.MemberIndex
					: -Sill.MemberIndex - 1);
		}
		for (const FExteriorColumnPlan& Column : Plan.ExteriorColumns)
		{
			Canonical += FString::Printf(
				TEXT("|EC:%d:F%d:C%d:M%u:N%lld:T%lld:LF%d:UF%d:G%d:S%d"),
				Column.ExteriorColumnId, Column.FacadePartitionId,
				Column.ComponentId, Column.FaceMask,
				QHash(Column.FacadeCoordinateCM),
				QHash(Column.TangentCoordinateCM),
				Column.LowerExteriorFrameId, Column.UpperExteriorFrameId,
				Column.bGroundToFirstFrame ? 1 : 0,
				Column.GroundSillSegmentId);
		}
		for (int32 Index = Stage2MemberCount; Index < Plan.Members.Num(); ++Index)
		{
			AppendMemberCanonical(Canonical, Plan.Members[Index]);
		}
		Plan.Summary.Stage3PlanHash = HashText(Canonical);
		Plan.Summary.SupportPlanHash = HashText(FString::Printf(
			TEXT("Stage2Support=%lld:Stage3=%lld"),
			Plan.Summary.SupportPlanHash, Plan.Summary.Stage3PlanHash));
		Plan.Summary.FinalGeometryHash = HashText(FString::Printf(
			TEXT("Stage2Geometry=%lld:Stage3=%lld"),
			Stage2GeometryHash, Plan.Summary.Stage3PlanHash));
		Plan.Summary.Stage3TotalMilliseconds = ElapsedMilliseconds(Stage3StartSeconds);
		return true;
	}
...
bool FABTSM73BeamC3V3SkeletonFirstGenerator::GenerateStage3(
	const FABTSM73BeamD0ResolvedProfile& Profile,
	const FABTSM73DAG5BV2GenerationResult& Silhouette,
	ABTSM73BeamC3V3::FGenerationResult& OutResult,
	FString& OutError) const
{
	...
	OutResult = FGenerationResult();
	OutError.Reset();
	if (!GenerateStage2(Profile, Silhouette, OutResult, OutError))
	{
		return false;
	}
	FPlan& Plan = OutResult.Plan;
	...
	const int32 Stage2MemberCount = Plan.Members.Num();
	const int64 Stage2GeometryHash = Plan.Summary.FinalGeometryHash;
	if (Assembly.Members.Num() != Stage2MemberCount
		|| !AppendStage3CommonExteriorFrame(Profile, Plan, OutError))
	{
		return false;
	}
	if (Plan.Summary.Stage3InputGeometryHash != Stage2GeometryHash)
	{
		OutError = TEXT("BeamC3V3Stage3InputIdentityDrift");
		return false;
	}
	...
	for (int32 PlannedIndex = Stage2MemberCount;
		PlannedIndex < Plan.Members.Num(); ++PlannedIndex)
	{
		const FPlannedMember& Planned = Plan.Members[PlannedIndex];
		if (!Assembly.Assemblies.IsValidIndex(Planned.ComponentId))
		{
			OutError = FString::Printf(
				TEXT("BeamC3V3Stage3InvalidOwner:Member=%d:Component=%d"),
				PlannedIndex, Planned.ComponentId);
			return false;
		}
		FABTSM73BeamAMember& Member = Assembly.Members.AddDefaulted_GetRef();
	...
	{
		OutError = FString::Printf(
			TEXT("BeamC3V3Stage3PlannedEmittedIRMismatch:Joints=%d/%d:Members=%d/%d"),
			Assembly.Joints.Num(), Plan.Summary.PlannedJointCount,
			Assembly.Members.Num(), Plan.Members.Num());
		return false;
	}
	...
	{
		OutError = FString::Printf(
			TEXT("BeamC3V3Stage3SeatVerificationFailed:Verified=%d:Expected=%d:Mismatch=%d"),
			Plan.Summary.VerifiedSeatCount,
			Plan.Summary.GroundSeatCount + Plan.Summary.PlannedSeatCount,
			Plan.Summary.SeatMismatchCount);
		return false;
...
bool FABTSM73BeamC3V3SkeletonFirstGenerator::GenerateStage4FloorTopFrames(
	const FABTSM73BeamD0ResolvedProfile& Profile,
	const FABTSM73DAG5BV2GenerationResult& Silhouette,
	ABTSM73BeamC3V3::FGenerationResult& OutResult,
	FString& OutError) const
{
	using namespace ABTSM73BeamC3V3;
	if (!GenerateStage4TopSurfaceIntent(Profile, Silhouette, OutResult, OutError))
	{
		return false;
	}
	FPlan& Plan = OutResult.Plan;
	...
	const double StartSeconds = FPlatformTime::Seconds();
	const int32 Stage3MemberCount = Plan.Members.Num();
	Plan.TopSurfaceFrameSegments.Reset();
	Plan.TopSurfaceFrameDeferredJunctions.Reset();
	Plan.Summary.Stage4TopFrameSegmentCount = 0;
	Plan.Summary.Stage4EmittedTopFrameSegmentCount = 0;
	Plan.Summary.Stage4ReusedTopFrameSegmentCount = 0;
	Plan.Summary.Stage4TopFrameBindingViolationCount = 0;
	Plan.Summary.Stage4TopFrameConflictCount = 0;
	Plan.Summary.Stage4DeferredFacadeColumnJunctionCount = 0;
	Plan.Summary.Stage4TopFrameHash = 0;
	Plan.Summary.Stage4TopFrameMilliseconds = 0.0;

	struct FTopFrameCell
	{
		int32 ComponentId = INDEX_NONE;
		int32 EnvelopeVolumeId = INDEX_NONE;
		int32 SourceVolumeId = INDEX_NONE;
		int32 SurfaceCourse = INDEX_NONE;
		uint8 FaceMask = 0;
		EABTSM73BeamAFrameAxis Axis = EABTSM73BeamAFrameAxis::X;
		double NormalCM = 0.0;
		double MinimumCM = 0.0;
		double MaximumCM = 0.0;
		TArray<int32> IntentIds;
	};
	TMap<FString, FTopFrameCell> CellsByKey;
	for (const FTopSurfaceIntentPlan& Intent : Plan.TopSurfaceIntents)
	{
		if (Intent.Intent != EFacadeDownwardIntent::TopSurface)
		{
		...
		const bool bNormalX = Intent.FaceMask == NegativeX
			|| Intent.FaceMask == PositiveX;
		const bool bPositive = Intent.FaceMask == PositiveX
			|| Intent.FaceMask == PositiveY;
		const EABTSM73BeamAFrameAxis Axis = bNormalX
			? EABTSM73BeamAFrameAxis::Y : EABTSM73BeamAFrameAxis::X;
		const double Normal = Intent.FacadeCoordinateCM
			+ (bPositive ? 1.0 : -1.0) * BlockUnitsCM * 0.5
				* (Intent.TopSurfaceAuthority
					== ETopSurfaceAuthorityKind::ExposedSetbackTop ? 1.0 : -1.0);
		const double Minimum = CellFaceAtOrAbove(
			Intent.TargetSupportTangentMinimumCM);
		const double Maximum = CellFaceAtOrBelow(
			Intent.TargetSupportTangentMaximumCM);
		if (Intent.TargetSurfaceCourseIndex == INDEX_NONE
			|| Maximum - Minimum < BlockUnitsCM - GeometryToleranceCM)
		{
			++Plan.Summary.Stage4TopFrameBindingViolationCount;
			continue;
		}
		for (double CellMinimum = Minimum;
			CellMinimum < Maximum - GeometryToleranceCM;
			CellMinimum += BlockUnitsCM)
		{
			...
			const FString Key = FString::Printf(
				TEXT("%d:%d:%d:%lld:%lld"), Intent.ComponentId,
				Intent.TargetSurfaceCourseIndex, static_cast<int32>(Axis),
				QHash(Normal), QHash(CellMinimum));
			FTopFrameCell& Cell = CellsByKey.FindOrAdd(Key);
			if (Cell.ComponentId == INDEX_NONE)
			{
				Cell.ComponentId = Intent.ComponentId;
				Cell.EnvelopeVolumeId = Intent.TargetEnvelopeVolumeId;
				Cell.SourceVolumeId = Intent.TargetSourceVolumeId;
				Cell.SurfaceCourse = Intent.TargetSurfaceCourseIndex;
				Cell.FaceMask = Intent.FaceMask;
				Cell.Axis = Axis;
				Cell.NormalCM = Normal;
				Cell.MinimumCM = CellMinimum;
				Cell.MaximumCM = CellMaximum;
			}
			else
			{
				Cell.FaceMask |= Intent.FaceMask;
				if (Intent.TargetEnvelopeVolumeId < Cell.EnvelopeVolumeId)
				{
					Cell.EnvelopeVolumeId = Intent.TargetEnvelopeVolumeId;
					Cell.SourceVolumeId = Intent.TargetSourceVolumeId;
				}
			}
			Cell.IntentIds.AddUnique(Intent.IntentId);
		}
	}
	TArray<FTopFrameCell> Cells;
	CellsByKey.GenerateValueArray(Cells);
	Cells.Sort([](const FTopFrameCell& A, const FTopFrameCell& B)
	{
		if (A.ComponentId != B.ComponentId) return A.ComponentId < B.ComponentId;
		if (A.SurfaceCourse != B.SurfaceCourse) return A.SurfaceCourse < B.SurfaceCourse;
		if (A.Axis != B.Axis) return A.Axis == EABTSM73BeamAFrameAxis::X;
		if (!FMath::IsNearlyEqual(A.NormalCM, B.NormalCM, GeometryToleranceCM))
		{
			return A.NormalCM < B.NormalCM;
		}
		return A.MinimumCM < B.MinimumCM;
	});

	auto AddLedger = [&Plan](const FTopFrameCell& Cell,
		const double Minimum, const double Maximum, const int32 MemberIndex,
		const bool bReused)
	{
		FTopSurfaceFrameSegmentPlan& Segment =
			Plan.TopSurfaceFrameSegments.AddDefaulted_GetRef();
		Segment.TopSurfaceFrameSegmentId =
			Plan.TopSurfaceFrameSegments.Num() - 1;
		Segment.SourceIntentIds = Cell.IntentIds;
		Segment.SourceIntentIds.Sort();
		Segment.ComponentId = Cell.ComponentId;
		Segment.TargetEnvelopeVolumeId = Cell.EnvelopeVolumeId;
		Segment.TargetSourceVolumeId = Cell.SourceVolumeId;
		Segment.SurfaceCourseIndex = Cell.SurfaceCourse;
		Segment.FaceMask = Cell.FaceMask;
		Segment.Axis = Cell.Axis;
		Segment.NormalCoordinateCM = Cell.NormalCM;
		Segment.TangentMinimumCM = Minimum;
		Segment.TangentMaximumCM = Maximum;
		Segment.MemberIndex = MemberIndex;
		Segment.bReusesExistingMember = bReused;
	};
	auto MakeMember = [&Plan](const FTopFrameCell& Cell,
		const double Minimum, const double Maximum)
	{
		const double GroundZ = Plan.Components.IsValidIndex(Cell.ComponentId)
			? Plan.Components[Cell.ComponentId].GroundPlaneZCM : 0.0;
		const double Z = GroundZ + (Cell.SurfaceCourse + 0.5) * BlockUnitsCM;
		FPlannedMember Member;
		Member.Axis = Cell.Axis;
		Member.LocalStart = Cell.Axis == EABTSM73BeamAFrameAxis::X
			? Position(Minimum, Cell.NormalCM, Z)
			: Position(Cell.NormalCM, Minimum, Z);
		Member.LocalEnd = Cell.Axis == EABTSM73BeamAFrameAxis::X
			? Position(Maximum, Cell.NormalCM, Z)
			: Position(Cell.NormalCM, Maximum, Z);
		return Member;
	};
	auto FindCoverOrConflict = [&Plan](const FPlannedMember& Probe,
		int32& OutCoveringMember)
	{
		OutCoveringMember = INDEX_NONE;
		const FBox Bounds = PlannedMemberBounds(Probe);
		for (int32 ExistingIndex = 0; ExistingIndex < Plan.Members.Num(); ++ExistingIndex)
		{
			const FPlannedMember& Existing = Plan.Members[ExistingIndex];
			const FBox ExistingBounds = PlannedMemberBounds(Existing);
			if (!BoxesPenetrate(Bounds, ExistingBounds))
			{
				continue;
			}
			const bool bSameSlab = Existing.Axis != EABTSM73BeamAFrameAxis::Z
				&& FMath::Abs(ExistingBounds.Min.Z - Bounds.Min.Z)
					<= GeometryToleranceCM
				&& FMath::Abs(ExistingBounds.Max.Z - Bounds.Max.Z)
					<= GeometryToleranceCM;
			const bool bCoversInSlab = bSameSlab
				&& ExistingBounds.Min.X <= Bounds.Min.X + GeometryToleranceCM
				&& ExistingBounds.Max.X >= Bounds.Max.X - GeometryToleranceCM
				&& ExistingBounds.Min.Y <= Bounds.Min.Y + GeometryToleranceCM
				&& ExistingBounds.Max.Y >= Bounds.Max.Y - GeometryToleranceCM;
			// A perpendicular horizontal member which already crosses the same
			// 36 cm cell is the real corner/junction material. Adopt it just like
			// Stage 3 does; emitting a duplicate would create positive volume.
			if (bCoversInSlab || bSameSlab)
			{
				OutCoveringMember = ExistingIndex;
				return false;
			}
			OutCoveringMember = ExistingIndex;
			return true;
	...
	{
		const FTopFrameCell& First = Cells[Cursor];
		int32 End = Cursor + 1;
		while (End < Cells.Num()
			&& Cells[End].ComponentId == First.ComponentId
			&& Cells[End].SurfaceCourse == First.SurfaceCourse
			&& Cells[End].Axis == First.Axis
			&& FMath::IsNearlyEqual(Cells[End].NormalCM, First.NormalCM,
				GeometryToleranceCM)
			&& FMath::IsNearlyEqual(Cells[End].MinimumCM,
				Cells[End - 1].MaximumCM, GeometryToleranceCM))
		{
			++End;
		}
		double FreeMinimum = 0.0;
		...
		auto FlushFreeRun = [&]() -> bool
		{
			...
			double RunCursor = FreeMinimum;
			while (RunCursor < FreeMaximum - GeometryToleranceCM)
			{
				const double RunEnd = FMath::Min(RunCursor
					+ MaximumHorizontalUnits * static_cast<double>(BlockUnitsCM),
					FreeMaximum);
				const FPlannedMember Probe = MakeMember(FreeCell, RunCursor, RunEnd);
				const FBox Bounds = PlannedMemberBounds(Probe);
				if (Plan.ReservedSupportVoids.ContainsByPredicate(
					[&Bounds](const FABTSM73BeamASupportVoid& Void)
					{
						return BoxesPenetrate(Bounds, Void.Bounds);
					}))
				{
					++Plan.Summary.Stage4TopFrameConflictCount;
					OutError = FString::Printf(
						TEXT("BeamC3V3Stage4TopFrameProtectedVoid:Component=%d:Course=%d:Axis=%d:Normal=%.3f:Run=%.3f..%.3f"),
						FreeCell.ComponentId, FreeCell.SurfaceCourse,
						static_cast<int32>(FreeCell.Axis), FreeCell.NormalCM,
						RunCursor, RunEnd);
					return false;
				}
				const int32 SegmentId = Plan.TopSurfaceFrameSegments.Num();
				const int32 MemberIndex = Plan.Members.Num();
				if (!AddPlannedMember(Plan, EOwnerKind::Floor,
					ESkeletonMemberKind::FloorCourse, FreeCell.ComponentId,
					FreeCell.ComponentId, FreeCell.SourceVolumeId, INDEX_NONE,
					FreeCell.SurfaceCourse,
					FMath::RoundToInt(RunCursor / BlockUnitsCM),
					FMath::RoundToInt(RunEnd / BlockUnitsCM), FreeCell.FaceMask,
					FreeCell.Axis, FreeCell.Axis == EABTSM73BeamAFrameAxis::X
						? EABTSM73BeamAMemberRole::PrimaryBeam
						: EABTSM73BeamAMemberRole::SecondaryBeam,
					Probe.LocalStart, Probe.LocalEnd, OutError))
				{
					return false;
				}
				Plan.Members[MemberIndex].OwnerId = FreeCell.ComponentId;
				AddLedger(FreeCell, RunCursor, RunEnd, MemberIndex, false);
				Plan.Members[MemberIndex].OwnerId = FreeCell.ComponentId;
				Plan.Members[MemberIndex].TargetFacadeSourceVolumeId =
					FreeCell.SourceVolumeId;
				Plan.Members[MemberIndex].TopSurfaceFrameSegmentId = SegmentId;
				Plan.Summary.TotalMemberLengthCM += RunEnd - RunCursor;
		...
		for (int32 CellIndex = Cursor; CellIndex < End; ++CellIndex)
		{
			const FTopFrameCell& Cell = Cells[CellIndex];
			const FPlannedMember Probe = MakeMember(
				Cell, Cell.MinimumCM, Cell.MaximumCM);
			const FBox Bounds = PlannedMemberBounds(Probe);
			if (Plan.ReservedSupportVoids.ContainsByPredicate(
				[&Bounds](const FABTSM73BeamASupportVoid& Void)
				{
				...
				}))
			{
				++Plan.Summary.Stage4TopFrameConflictCount;
				OutError = FString::Printf(
					TEXT("BeamC3V3Stage4TopFrameProtectedVoidCell:Component=%d:Course=%d:Axis=%d:Normal=%.3f:Cell=%.3f..%.3f"),
					Cell.ComponentId, Cell.SurfaceCourse,
					static_cast<int32>(Cell.Axis), Cell.NormalCM,
					Cell.MinimumCM, Cell.MaximumCM);
				return false;
			}
			int32 CoveringMember = INDEX_NONE;
			if (FindCoverOrConflict(Probe, CoveringMember))
			{
				const FPlannedMember* Blocker =
					Plan.Members.IsValidIndex(CoveringMember)
						? &Plan.Members[CoveringMember] : nullptr;
				const bool bDeferredStage3FacadeColumn = Blocker != nullptr
					&& Blocker->Axis == EABTSM73BeamAFrameAxis::Z
					&& (Blocker->SkeletonKind == ESkeletonMemberKind::ExteriorPost
						|| Blocker->SkeletonKind
							== ESkeletonMemberKind::GroundExteriorPost);
				if (bDeferredStage3FacadeColumn)
				{
					if (!FlushFreeRun()) return false;
					FTopSurfaceFrameDeferredJunctionPlan& Junction =
						Plan.TopSurfaceFrameDeferredJunctions.AddDefaulted_GetRef();
					Junction.DeferredJunctionId =
						Plan.TopSurfaceFrameDeferredJunctions.Num() - 1;
					Junction.SourceIntentIds = Cell.IntentIds;
					Junction.SourceIntentIds.Sort();
					Junction.ComponentId = Cell.ComponentId;
					Junction.SurfaceCourseIndex = Cell.SurfaceCourse;
					Junction.FaceMask = Cell.FaceMask;
					Junction.Axis = Cell.Axis;
					Junction.NormalCoordinateCM = Cell.NormalCM;
					Junction.TangentMinimumCM = Cell.MinimumCM;
					Junction.TangentMaximumCM = Cell.MaximumCM;
					Junction.BlockingStage3ColumnMemberIndex = CoveringMember;
					++Plan.Summary.Stage4DeferredFacadeColumnJunctionCount;
					continue;
				}
				++Plan.Summary.Stage4TopFrameConflictCount;
				OutError = FString::Printf(
					TEXT("BeamC3V3Stage4TopFrameMemberConflict:Component=%d:Course=%d:Axis=%d:Normal=%.3f:Cell=%.3f..%.3f:Blocker=%d:Kind=%d:BlockerAxis=%d:BlockerCourse=%d"),
					Cell.ComponentId, Cell.SurfaceCourse,
					static_cast<int32>(Cell.Axis), Cell.NormalCM,
					Cell.MinimumCM, Cell.MaximumCM, CoveringMember,
					Blocker != nullptr ? static_cast<int32>(Blocker->SkeletonKind)
						: INDEX_NONE,
					Blocker != nullptr ? static_cast<int32>(Blocker->Axis) : INDEX_NONE,
					Blocker != nullptr ? Blocker->CourseIndex : INDEX_NONE);
				return false;
			}
			if (CoveringMember != INDEX_NONE)
			{
				if (!FlushFreeRun()) return false;
				AddLedger(Cell, Cell.MinimumCM, Cell.MaximumCM,
					CoveringMember, true);
				++Plan.Summary.Stage4ReusedTopFrameSegmentCount;
				continue;
			}
			if (!bHasFreeRun)
			{
				FreeMinimum = Cell.MinimumCM;
				FreeCell = Cell;
				bHasFreeRun = true;
			}
			FreeMaximum = Cell.MaximumCM;
			for (const int32 IntentId : Cell.IntentIds)
			{
				FreeCell.IntentIds.AddUnique(IntentId);
			}
		}
		if (!FlushFreeRun()) return false;
		Cursor = End;
	...
	Plan.Summary.Stage4TopFrameSegmentCount =
		Plan.TopSurfaceFrameSegments.Num();
	TSet<int32> BoundIntentIds;
	FString Canonical = FString::Printf(TEXT("Intent=%lld"),
		Plan.Summary.Stage4IntentHash);
	for (const FTopSurfaceFrameSegmentPlan& Segment
		: Plan.TopSurfaceFrameSegments)
	{
		const bool bValid = Plan.Members.IsValidIndex(Segment.MemberIndex)
			&& !Segment.SourceIntentIds.IsEmpty()
			&& Segment.SurfaceCourseIndex != INDEX_NONE
			&& Segment.TangentMaximumCM - Segment.TangentMinimumCM
				>= BlockUnitsCM - GeometryToleranceCM;
		Plan.Summary.Stage4TopFrameBindingViolationCount += bValid ? 0 : 1;
		for (const int32 IntentId : Segment.SourceIntentIds)
		{
			BoundIntentIds.Add(IntentId);
		}
		Canonical += FString::Printf(
			TEXT("|TF:%d:C%d:E%d:S%d:Q%d:M%u:A%d:N%lld:TA%lld:TB%lld:R%d:I"),
			Segment.TopSurfaceFrameSegmentId, Segment.ComponentId,
			Segment.TargetEnvelopeVolumeId, Segment.TargetSourceVolumeId,
			Segment.SurfaceCourseIndex, Segment.FaceMask,
			static_cast<int32>(Segment.Axis), QHash(Segment.NormalCoordinateCM),
			QHash(Segment.TangentMinimumCM), QHash(Segment.TangentMaximumCM),
			Segment.bReusesExistingMember ? Segment.MemberIndex
				: -Segment.MemberIndex - 1);
		for (const int32 IntentId : Segment.SourceIntentIds)
		{
			Canonical += FString::Printf(TEXT(",%d"), IntentId);
		}
	}
	for (const FTopSurfaceFrameDeferredJunctionPlan& Junction
		: Plan.TopSurfaceFrameDeferredJunctions)
	{
		Canonical += FString::Printf(
			TEXT("|DJ:%d:C%d:Q%d:M%u:A%d:N%lld:TA%lld:TB%lld:B%d:I"),
			Junction.DeferredJunctionId, Junction.ComponentId,
			Junction.SurfaceCourseIndex, Junction.FaceMask,
			static_cast<int32>(Junction.Axis), QHash(Junction.NormalCoordinateCM),
			QHash(Junction.TangentMinimumCM), QHash(Junction.TangentMaximumCM),
			Junction.BlockingStage3ColumnMemberIndex);
		for (const int32 IntentId : Junction.SourceIntentIds)
		{
			Canonical += FString::Printf(TEXT(",%d"), IntentId);
			BoundIntentIds.Add(IntentId);
		}
	...
	{
		if (Intent.Intent == EFacadeDownwardIntent::TopSurface
			&& !BoundIntentIds.Contains(Intent.IntentId))
		{
			++Plan.Summary.Stage4TopFrameBindingViolationCount;
		}
	}
	if (Plan.Summary.Stage4ResolvedTopSurfaceIntentCount > 0
		&& (Plan.Summary.Stage4TopFrameSegmentCount <= 0
			|| Plan.Summary.Stage4TopFrameBindingViolationCount != 0
			|| Plan.Summary.Stage4TopFrameConflictCount != 0))
	{
		OutError = FString::Printf(
			TEXT("BeamC3V3Stage4TopFrameContractFailed:Top=%d:Segments=%d:Emitted=%d:Reused=%d:BindingViolations=%d:Conflicts=%d"),
			Plan.Summary.Stage4ResolvedTopSurfaceIntentCount,
			Plan.Summary.Stage4TopFrameSegmentCount,
			Plan.Summary.Stage4EmittedTopFrameSegmentCount,
			Plan.Summary.Stage4ReusedTopFrameSegmentCount,
	...
	Plan.Summary.Stage4TopFrameHash = HashText(Canonical);
	Plan.Summary.FinalGeometryHash = HashText(FString::Printf(
		TEXT("Stage3=%lld:Stage4TopFrames=%lld"),
		Plan.Summary.Stage3PlanHash, Plan.Summary.Stage4TopFrameHash));
	Plan.Summary.PlannedMemberCount = Plan.Members.Num();
	if (!ValidatePreflightCaps(Profile.BeamSettings.BeamB.BeamA, Plan, OutError))
	{
		return false;
	}
	...
	for (int32 MemberIndex = Stage3MemberCount;
		MemberIndex < Plan.Members.Num(); ++MemberIndex)
	{
		const FPlannedMember& Planned = Plan.Members[MemberIndex];
		if (!Assembly.Assemblies.IsValidIndex(Planned.ComponentId))
		{
			OutError = FString::Printf(
				TEXT("BeamC3V3Stage4TopFrameInvalidOwner:Member=%d:Component=%d"),
				MemberIndex, Planned.ComponentId);
			return false;
		}
		FABTSM73BeamAMember& Member = Assembly.Members.AddDefaulted_GetRef();
		Member.MemberId = Assembly.Members.Num() - 1;
	...
	{
		OutError = FString::Printf(
			TEXT("BeamC3V3Stage4TopFrameIRMismatch:Joints=%d/%d:Members=%d/%d"),
			Assembly.Joints.Num(), Plan.Summary.PlannedJointCount,
			Assembly.Members.Num(), Plan.Members.Num());
		return false;
	}
	...
	Plan.Summary.EmittedMemberCount = Assembly.Members.Num();
	Plan.Summary.bAccepted = true;
	Plan.Summary.Stage4TopFrameMilliseconds = ElapsedMilliseconds(StartSeconds);
	return Plan.Summary.Stage4TopFrameHash != 0;
}

bool FABTSM73BeamC3V3SkeletonFirstGenerator::GenerateStage4FacadeToTopConnections(
	const FABTSM73BeamD0ResolvedProfile& Profile,
	const FABTSM73DAG5BV2GenerationResult& Silhouette,
	ABTSM73BeamC3V3::FGenerationResult& OutResult,
	FString& OutError) const
{
	using namespace ABTSM73BeamC3V3;
	if (!GenerateStage4FloorTopFrames(Profile, Silhouette, OutResult, OutError))
	{
		return false;
	}
	FPlan& Plan = OutResult.Plan;
	FABTSM73BeamAGenerationResult& Assembly = OutResult.Assembly;
	const double StartSeconds = FPlatformTime::Seconds();
	const int32 InputMemberCount = Plan.Members.Num();
	Plan.FacadeToTopConnections.Reset();
	Plan.Summary.Stage4FacadeToTopConnectionCount = 0;
	Plan.Summary.Stage4FacadeToTopSeatCount = 0;
	Plan.Summary.Stage4FacadeToTopPostSegmentCount = 0;
	Plan.Summary.Stage4SuppressedStage3ColumnMemberCount = 0;
	Plan.Summary.Stage4ResolvedDeferredJunctionCount = 0;
	...
	Plan.Summary.Stage4FacadeToTopMilliseconds = 0.0;
	FString BindingDiagnostics;

	auto FindFrame = [&Plan](const int32 ExteriorFrameId)
	{
		return Plan.CommonExteriorFrames.FindByPredicate(
			[ExteriorFrameId](const FCommonExteriorFramePlan& Frame)
			{
				return Frame.ExteriorFrameId == ExteriorFrameId;
			});
	};
	auto FindTopFrameForIntent = [&Plan](const int32 IntentId,
		const double PreferredStation)
	{
		...
		for (const FTopSurfaceFrameSegmentPlan& Segment : Plan.TopSurfaceFrameSegments)
		{
			...
			const double Distance = PreferredStation < Segment.TangentMinimumCM
	...
	TSet<int32> GroundedFrameMemberIndices;
	for (const FTopSurfaceIntentPlan& Intent : Plan.TopSurfaceIntents)
	{
		if (Intent.Intent == EFacadeDownwardIntent::GroundSill)
		{
			if (const FCommonExteriorFramePlan* Frame = FindFrame(Intent.ExteriorFrameId))
			{
				GroundedFrameMemberIndices.Add(Frame->MemberIndex);
			}
		}
	}
	...
	for (const FTopSurfaceIntentPlan& Intent : Plan.TopSurfaceIntents)
	{
		...
		const FCommonExteriorFramePlan* Frame = FindFrame(Intent.ExteriorFrameId);
		if (Frame == nullptr || GroundedFrameMemberIndices.Contains(Frame->MemberIndex))
		{
			continue;
		}
		for (const FExteriorColumnPlan& Column : Plan.ExteriorColumns)
		{
			if (Column.UpperExteriorFrameId != Intent.ExteriorFrameId)
			{
				continue;
			}
	...
	auto PenetratesActiveMember = [&Plan](const FPlannedMember& Probe,
		const TSet<int32>& AllowedMembers, int32& OutBlocker)
	{
		OutBlocker = INDEX_NONE;
		const FBox Bounds = PlannedMemberBounds(Probe);
		double NearestBlockerZ = DBL_MAX;
		for (int32 MemberIndex = 0; MemberIndex < Plan.Members.Num(); ++MemberIndex)
		{
			if (AllowedMembers.Contains(MemberIndex)
				|| Plan.Members[MemberIndex].bSuppressedByStage4FacadeToTop)
			{
				continue;
			}
			if (BoxesPenetrate(Bounds, PlannedMemberBounds(Plan.Members[MemberIndex])))
			{
				const double CandidateZ = PlannedMemberBounds(
					Plan.Members[MemberIndex]).Min.Z;
				if (Probe.Axis != EABTSM73BeamAFrameAxis::Z)
				{
					OutBlocker = MemberIndex;
					return true;
				}
				if (CandidateZ < NearestBlockerZ - GeometryToleranceCM
					|| (FMath::IsNearlyEqual(CandidateZ, NearestBlockerZ,
						GeometryToleranceCM) && MemberIndex < OutBlocker))
				{
					OutBlocker = MemberIndex;
					NearestBlockerZ = CandidateZ;
				}
			}
		}
		return OutBlocker != INDEX_NONE;
	};
	auto AddConnectionMember = [&Plan, &OutError](
		const ESkeletonMemberKind Kind, const int32 ConnectionId,
		const int32 ComponentId, const int32 SourceVolumeId,
		const int32 CourseIndex, const int32 StationA, const int32 StationB,
		const uint8 FaceMask, const EABTSM73BeamAFrameAxis Axis,
		const FVector& Start, const FVector& End, int32& OutMemberIndex)
	{
		OutMemberIndex = Plan.Members.Num();
		if (!AddPlannedMember(Plan, EOwnerKind::Floor, Kind, ConnectionId,
			ComponentId, SourceVolumeId, INDEX_NONE, CourseIndex, StationA,
			StationB, FaceMask, Axis,
			Axis == EABTSM73BeamAFrameAxis::Z
				? EABTSM73BeamAMemberRole::Post
				: EABTSM73BeamAMemberRole::PrimaryBeam,
			Start, End, OutError))
		...
		Plan.Members[OutMemberIndex].FacadeToTopConnectionId = ConnectionId;
		Plan.Summary.TotalMemberLengthCM += FVector::Distance(Start, End);
		return true;
	};
	...
	for (FTopSurfaceFrameDeferredJunctionPlan& Junction
		: Plan.TopSurfaceFrameDeferredJunctions)
	{
		if (!Plan.Members.IsValidIndex(Junction.BlockingStage3ColumnMemberIndex)
			|| !Plan.Members[Junction.BlockingStage3ColumnMemberIndex]
	...
	for (const FTopSurfaceIntentPlan& Intent : Plan.TopSurfaceIntents)
	{
		...
		const FCommonExteriorFramePlan* UpperFrame = FindFrame(Intent.ExteriorFrameId);
		if (UpperFrame == nullptr || !Plan.Members.IsValidIndex(UpperFrame->MemberIndex))
		{
			++Plan.Summary.Stage4FacadeToTopBindingViolationCount;
			BindingDiagnostics += FString::Printf(
				TEXT("|MissingUpperIntent%d:Frame=%d"), Intent.IntentId,
				Intent.ExteriorFrameId);
			continue;
		}
		if (GroundedFrameMemberIndices.Contains(UpperFrame->MemberIndex))
		{
			FFacadeToTopConnectionPlan& Connection =
				Plan.FacadeToTopConnections.AddDefaulted_GetRef();
			Connection.ConnectionId = Plan.FacadeToTopConnections.Num() - 1;
			Connection.SourceIntentIds.Add(Intent.IntentId);
			Connection.ComponentId = Intent.ComponentId;
			Connection.UpperExteriorFrameId = Intent.ExteriorFrameId;
			Connection.UpperExteriorFrameMemberIndex = UpperFrame->MemberIndex;
			Connection.TopSurfaceAuthority = Intent.TopSurfaceAuthority;
			Connection.bReusesGroundedAlias = true;
			continue;
		}

		TArray<const FExteriorColumnPlan*> SourceColumns;
		for (const FExteriorColumnPlan& Column : Plan.ExteriorColumns)
		{
			if (Column.UpperExteriorFrameId == Intent.ExteriorFrameId)
			{
		...
		{
			SourceColumns.Add(nullptr);
		}
		for (const FExteriorColumnPlan* SourceColumn : SourceColumns)
		{
			const double PreferredStation = SourceColumn != nullptr
				? SourceColumn->TangentCoordinateCM
				: (Intent.TargetSupportTangentMinimumCM
					+ Intent.TargetSupportTangentMaximumCM) * 0.5;
			const FTopSurfaceFrameSegmentPlan* TopFrame =
				FindTopFrameForIntent(Intent.IntentId, PreferredStation);
			if (TopFrame == nullptr)
			{
				++Plan.Summary.Stage4FacadeToTopBindingViolationCount;
				BindingDiagnostics += FString::Printf(
			...
			// zero; re-snapping here can move a previously legal facade post into a
			// ThroughCourse.
			double Station = FMath::Clamp(PreferredStation,
				TopFrame->TangentMinimumCM + BlockUnitsCM * 0.5,
				TopFrame->TangentMaximumCM - BlockUnitsCM * 0.5);
			int32 SeatLiftCourseCount = 1;
			int32 ReusedSeatMemberIndex = INDEX_NONE;
			TArray<int32> PreexistingSupportChain;
			const bool bZeroClearanceSetback = Intent.TopSurfaceAuthority
				== ETopSurfaceAuthorityKind::ExposedSetbackTop
				&& Intent.ExteriorFrameCourseIndex
					<= Intent.TargetSurfaceCourseIndex + 1;
			if (!bZeroClearanceSetback && Intent.TopSurfaceAuthority
				== ETopSurfaceAuthorityKind::ExposedSetbackTop)
			{
				const bool bNormalX = Intent.FaceMask == NegativeX
					|| Intent.FaceMask == PositiveX;
				const bool bPositive = Intent.FaceMask == PositiveX
					|| Intent.FaceMask == PositiveY;
				const double PostNormal = Intent.FacadeCoordinateCM
					+ (bPositive ? -1.0 : 1.0) * BlockUnitsCM * 0.5;
				const double SeatMinimum = FMath::Min(
					PostNormal, TopFrame->NormalCoordinateCM) - BlockUnitsCM * 0.5;
				...
				TArray<double> CandidateStations = {Station};
				for (double Candidate = TopFrame->TangentMinimumCM
						+ BlockUnitsCM * 0.5;
					Candidate <= TopFrame->TangentMaximumCM
						- BlockUnitsCM * 0.5 + GeometryToleranceCM;
					Candidate += BlockUnitsCM)
				{
					CandidateStations.AddUnique(Candidate);
				}
				bool bFoundSeatStation = false;
				FString SeatDiagnostics;
				const int32 MaximumLift = FMath::Clamp(
					Intent.ExteriorFrameCourseIndex
						- Intent.TargetSurfaceCourseIndex - 1, 1, 20);
				for (int32 Lift = 1; Lift <= MaximumLift && !bFoundSeatStation; ++Lift)
				{
					const double SeatZ = GroundZ
						+ (Intent.TargetSurfaceCourseIndex + Lift + 0.5)
							* BlockUnitsCM;
					for (const double Candidate : CandidateStations)
					{
						FPlannedMember SeatProbe;
						SeatProbe.Axis = bNormalX ? EABTSM73BeamAFrameAxis::X
							: EABTSM73BeamAFrameAxis::Y;
						SeatProbe.LocalStart = bNormalX
							? Position(SeatMinimum, Candidate, SeatZ)
							: Position(Candidate, SeatMinimum, SeatZ);
						SeatProbe.LocalEnd = bNormalX
							? Position(SeatMaximum, Candidate, SeatZ)
							: Position(Candidate, SeatMaximum, SeatZ);
						TSet<int32> Allowed = {
							TopFrame->MemberIndex, UpperFrame->MemberIndex};
						int32 Blocker = INDEX_NONE;
						if (!PenetratesActiveMember(SeatProbe, Allowed, Blocker))
						{
							Station = Candidate;
							SeatLiftCourseCount = Lift;
							bFoundSeatStation = true;
							break;
						}
						if (Plan.Members.IsValidIndex(Blocker)
							&& Plan.Members[Blocker].SkeletonKind
								== ESkeletonMemberKind::FacadeToTopSeat
							&& Plan.Members[Blocker].Axis == SeatProbe.Axis)
						{
							const FBox ExistingBounds = PlannedMemberBounds(
								Plan.Members[Blocker]);
							const FBox ProbeBounds = PlannedMemberBounds(SeatProbe);
							if (ExistingBounds.Min.X
									<= ProbeBounds.Min.X + GeometryToleranceCM
								&& ExistingBounds.Max.X
									>= ProbeBounds.Max.X - GeometryToleranceCM
								&& ExistingBounds.Min.Y
									<= ProbeBounds.Min.Y + GeometryToleranceCM
								&& ExistingBounds.Max.Y
									>= ProbeBounds.Max.Y - GeometryToleranceCM
								&& ExistingBounds.Min.Z
									<= ProbeBounds.Min.Z + GeometryToleranceCM
								&& ExistingBounds.Max.Z
									>= ProbeBounds.Max.Z - GeometryToleranceCM)
							{
								Station = Candidate;
								SeatLiftCourseCount = Lift;
								ReusedSeatMemberIndex = Blocker;
								bFoundSeatStation = true;
								break;
							}
						}
						if (SeatDiagnostics.Len() < 1200
							&& Plan.Members.IsValidIndex(Blocker))
						{
							const FBox BlockerBounds = PlannedMemberBounds(
								Plan.Members[Blocker]);
							SeatDiagnostics += FString::Printf(
								TEXT("|L%d:T%.1f:B%d:K%d:A%d:Q%d:%.1f,%.1f..%.1f,%.1f"),
								Lift, Candidate, Blocker,
								static_cast<int32>(Plan.Members[Blocker].SkeletonKind),
								static_cast<int32>(Plan.Members[Blocker].Axis),
								Plan.Members[Blocker].CourseIndex,
								BlockerBounds.Min.X, BlockerBounds.Min.Y,
								BlockerBounds.Max.X, BlockerBounds.Max.Y);
						}
					}
				}
				if (!bFoundSeatStation)
				{
					const double ChainStartZ = GroundZ
						+ (Intent.TargetSurfaceCourseIndex + 1) * BlockUnitsCM;
					const double ChainEndZ = GroundZ
						+ Intent.ExteriorFrameCourseIndex * BlockUnitsCM;
					for (const double Candidate : CandidateStations)
					{
						TArray<int32> CandidateChain;
						bool bComplete = true;
						for (double CellBottomZ = ChainStartZ;
							CellBottomZ < ChainEndZ - GeometryToleranceCM;
							CellBottomZ += BlockUnitsCM)
						{
							const double SampleZ = CellBottomZ + BlockUnitsCM * 0.5;
							const double X = bNormalX ? PostNormal : Candidate;
							const double Y = bNormalX ? Candidate : PostNormal;
							int32 CoveringMemberIndex = INDEX_NONE;
							for (int32 MemberIndex = 0;
								MemberIndex < Plan.Members.Num(); ++MemberIndex)
							{
								const FPlannedMember& Member = Plan.Members[MemberIndex];
								if (Member.ComponentId != Intent.ComponentId
									|| Member.bSuppressedByStage4FacadeToTop
									|| Member.Axis == EABTSM73BeamAFrameAxis::Z)
								{
									continue;
								}
								const FBox Bounds = PlannedMemberBounds(Member);
								if (X >= Bounds.Min.X - GeometryToleranceCM
									&& X <= Bounds.Max.X + GeometryToleranceCM
									&& Y >= Bounds.Min.Y - GeometryToleranceCM
									&& Y <= Bounds.Max.Y + GeometryToleranceCM
									&& SampleZ >= Bounds.Min.Z - GeometryToleranceCM
									&& SampleZ <= Bounds.Max.Z + GeometryToleranceCM)
								{
									CoveringMemberIndex = MemberIndex;
									break;
								}
							}
							if (CoveringMemberIndex == INDEX_NONE)
							{
								bComplete = false;
								break;
							}
							CandidateChain.AddUnique(CoveringMemberIndex);
						}
						if (bComplete && !CandidateChain.IsEmpty())
						{
							Station = Candidate;
							PreexistingSupportChain = MoveTemp(CandidateChain);
							break;
						}
					}
					if (PreexistingSupportChain.IsEmpty())
					{
						...
						OutError = FString::Printf(
							TEXT("BeamC3V3Stage4FacadeToTopSeatStationUnavailable:Intent=%d:Frame=%d:Range=%.3f..%.3f:Diagnostics=%s"),
							Intent.IntentId, Intent.ExteriorFrameId,
							TopFrame->TangentMinimumCM,
							TopFrame->TangentMaximumCM, *SeatDiagnostics);
						return false;
					}
				}
			}
			const FString Key = FString::Printf(TEXT("%d:%lld"),
				UpperFrame->MemberIndex, QHash(Station));
			if (EmittedConnectionKeys.Contains(Key))
			{
				if (FFacadeToTopConnectionPlan* ExistingConnection =
					Plan.FacadeToTopConnections.FindByPredicate(
						[UpperFrame, Station](const FFacadeToTopConnectionPlan& Candidate)
						{
							return Candidate.UpperExteriorFrameMemberIndex
								== UpperFrame->MemberIndex
								&& FMath::IsNearlyEqual(Candidate.TangentStationCM,
									Station, GeometryToleranceCM);
						}))
				{
					ExistingConnection->SourceIntentIds.AddUnique(Intent.IntentId);
				}
				continue;
			}
			...
			Connection.UpperExteriorFrameId = Intent.ExteriorFrameId;
			Connection.UpperExteriorFrameMemberIndex = UpperFrame->MemberIndex;
			Connection.TopSurfaceFrameSegmentId =
				TopFrame->TopSurfaceFrameSegmentId;
			Connection.TopSurfaceFrameMemberIndex = TopFrame->MemberIndex;
			Connection.TopSurfaceAuthority = Intent.TopSurfaceAuthority;
			...
			if (!PreexistingSupportChain.IsEmpty())
			{
				Connection.ReusedSupportMemberIndices =
					MoveTemp(PreexistingSupportChain);
				Connection.bReusesExistingSupportChain = true;
				continue;
			}
			if (bZeroClearanceSetback)
			{
				// There is no free 36 cm course between the exposed shoulder and
				// the facade frame.  The frame bears directly on the semantic top;
				// inserting a perpendicular seat would overlap the frame's own cell.
				Connection.bDirectFrameContact = true;
				continue;
			}
			...
			const double GroundZ = Plan.Components.IsValidIndex(Intent.ComponentId)
				? Plan.Components[Intent.ComponentId].GroundPlaneZCM : 0.0;
			const double UpperBottomZ = GroundZ
				+ Intent.ExteriorFrameCourseIndex * BlockUnitsCM;
			double PostStartZ = GroundZ
				+ (Intent.TargetSurfaceCourseIndex + 1) * BlockUnitsCM;
			double PostNormal = TopFrame->NormalCoordinateCM;
			int32 LowerMemberIndex = TopFrame->MemberIndex;
			if (Intent.TopSurfaceAuthority
				== ETopSurfaceAuthorityKind::ExposedSetbackTop)
			{
				PostNormal = Intent.FacadeCoordinateCM
					+ (bPositive ? -1.0 : 1.0) * BlockUnitsCM * 0.5;
				const double OuterNormal = TopFrame->NormalCoordinateCM;
				const double SeatMinimum = FMath::Min(PostNormal, OuterNormal)
					- BlockUnitsCM * 0.5;
				const double SeatMaximum = FMath::Max(PostNormal, OuterNormal)
					+ BlockUnitsCM * 0.5;
				const double SeatZ = GroundZ
					+ (Intent.TargetSurfaceCourseIndex
						+ SeatLiftCourseCount + 0.5) * BlockUnitsCM;
				FPlannedMember SeatProbe;
				SeatProbe.Axis = bNormalX ? EABTSM73BeamAFrameAxis::X
					: EABTSM73BeamAFrameAxis::Y;
				SeatProbe.LocalStart = bNormalX
					? Position(SeatMinimum, Station, SeatZ)
					: Position(Station, SeatMinimum, SeatZ);
				SeatProbe.LocalEnd = bNormalX
					? Position(SeatMaximum, Station, SeatZ)
					: Position(Station, SeatMaximum, SeatZ);
				TSet<int32> Allowed = {TopFrame->MemberIndex,
					UpperFrame->MemberIndex, ReusedSeatMemberIndex};
				int32 Blocker = INDEX_NONE;
				if (PenetratesActiveMember(SeatProbe, Allowed, Blocker))
				{
					++Plan.Summary.Stage4FacadeToTopConflictCount;
					OutError = FString::Printf(
						TEXT("BeamC3V3Stage4FacadeToTopSeatConflict:Intent=%d:Frame=%d:Station=%.3f:Blocker=%d"),
						Intent.IntentId, Intent.ExteriorFrameId, Station, Blocker);
					return false;
				}
				if (ReusedSeatMemberIndex == INDEX_NONE
					&& SeatLiftCourseCount > 1)
				{
					const double RiserStartZ = GroundZ
						+ (Intent.TargetSurfaceCourseIndex + 1) * BlockUnitsCM;
					const double RiserEndZ = GroundZ
						+ (Intent.TargetSurfaceCourseIndex + SeatLiftCourseCount)
							* BlockUnitsCM;
					FPlannedMember RiserProbe;
					RiserProbe.Axis = EABTSM73BeamAFrameAxis::Z;
					RiserProbe.LocalStart = bNormalX
						? Position(OuterNormal, Station, RiserStartZ)
						: Position(Station, OuterNormal, RiserStartZ);
					RiserProbe.LocalEnd = RiserProbe.LocalStart
						+ FVector(0.0, 0.0, RiserEndZ - RiserStartZ);
					TSet<int32> RiserAllowed = {
						TopFrame->MemberIndex, UpperFrame->MemberIndex};
					int32 RiserBlocker = INDEX_NONE;
					if (PenetratesActiveMember(RiserProbe, RiserAllowed, RiserBlocker))
					{
						++Plan.Summary.Stage4FacadeToTopConflictCount;
						OutError = FString::Printf(
							TEXT("BeamC3V3Stage4FacadeToTopOuterRiserConflict:Intent=%d:Frame=%d:Station=%.3f:Blocker=%d"),
							Intent.IntentId, Intent.ExteriorFrameId,
							Station, RiserBlocker);
						return false;
					}
					int32 RiserMemberIndex = INDEX_NONE;
					if (!AddConnectionMember(ESkeletonMemberKind::FacadeToTopPost,
						Connection.ConnectionId, Intent.ComponentId,
						Intent.TargetSourceVolumeId,
						Intent.TargetSurfaceCourseIndex + 1,
						FMath::RoundToInt(Station / BlockUnitsCM), INDEX_NONE,
						Intent.FaceMask, EABTSM73BeamAFrameAxis::Z,
						RiserProbe.LocalStart, RiserProbe.LocalEnd,
						RiserMemberIndex))
					{
						return false;
					}
					AddSeat(Plan, RiserMemberIndex, TopFrame->MemberIndex);
					Connection.PostSegmentMemberIndices.Add(RiserMemberIndex);
					LowerMemberIndex = RiserMemberIndex;
					++Plan.Summary.Stage4FacadeToTopPostSegmentCount;
				}
				if (ReusedSeatMemberIndex != INDEX_NONE)
				{
					Connection.SeatMemberIndex = ReusedSeatMemberIndex;
					Connection.bReusesExistingSeat = true;
					LowerMemberIndex = ReusedSeatMemberIndex;
				}
				else
				{
					if (!AddConnectionMember(ESkeletonMemberKind::FacadeToTopSeat,
						Connection.ConnectionId, Intent.ComponentId,
						Intent.TargetSourceVolumeId,
						Intent.TargetSurfaceCourseIndex + SeatLiftCourseCount,
						FMath::RoundToInt(SeatMinimum / BlockUnitsCM),
						FMath::RoundToInt(SeatMaximum / BlockUnitsCM), Intent.FaceMask,
						SeatProbe.Axis, SeatProbe.LocalStart, SeatProbe.LocalEnd,
						Connection.SeatMemberIndex))
					{
						return false;
					}
					AddSeat(Plan, Connection.SeatMemberIndex, LowerMemberIndex);
					LowerMemberIndex = Connection.SeatMemberIndex;
					++Plan.Summary.Stage4FacadeToTopSeatCount;
				}
				PostStartZ = GroundZ + (Intent.TargetSurfaceCourseIndex
					+ SeatLiftCourseCount + 1) * BlockUnitsCM;
			}

			if (PostStartZ > UpperBottomZ + GeometryToleranceCM)
			{
				if (Intent.TopSurfaceAuthority
					== ETopSurfaceAuthorityKind::DirectStackSeat
					&& Intent.TargetSurfaceCourseIndex
						>= Intent.ExteriorFrameCourseIndex)
				{
					// The partition-root frame already occupies the stacking seam;
					// there is no positive-height gap in which a post could exist.
					Connection.bDirectFrameContact = true;
					continue;
				}
				++Plan.Summary.Stage4FacadeToTopBindingViolationCount;
				BindingDiagnostics += FString::Printf(
					TEXT("|InvertedGapIntent%d:Authority=%d:Surface=%d:Lift=%d:Frame=%d"),
					Intent.IntentId, static_cast<int32>(Intent.TopSurfaceAuthority),
					Intent.TargetSurfaceCourseIndex, SeatLiftCourseCount,
					Intent.ExteriorFrameCourseIndex);
				continue;
			}
			if (FMath::IsNearlyEqual(PostStartZ, UpperBottomZ,
				GeometryToleranceCM))
			{
				Connection.bDirectFrameContact = true;
				continue;
			}
			double SegmentStartZ = PostStartZ;
			while (SegmentStartZ < UpperBottomZ - GeometryToleranceCM)
			{
				double SegmentEndZ = FMath::Min(
					UpperBottomZ, SegmentStartZ + 720.0);
				FPlannedMember PostProbe;
				PostProbe.Axis = EABTSM73BeamAFrameAxis::Z;
				PostProbe.LocalStart = bNormalX
					? Position(PostNormal, Station, SegmentStartZ)
					: Position(Station, PostNormal, SegmentStartZ);
				PostProbe.LocalEnd = PostProbe.LocalStart
					+ FVector(0.0, 0.0, SegmentEndZ - SegmentStartZ);
				TSet<int32> Allowed = {LowerMemberIndex, UpperFrame->MemberIndex};
				int32 Blocker = INDEX_NONE;
				if (PenetratesActiveMember(PostProbe, Allowed, Blocker))
				{
					const FPlannedMember* BlockingMember =
						Plan.Members.IsValidIndex(Blocker) ? &Plan.Members[Blocker] : nullptr;
					const FBox BlockingBounds = BlockingMember != nullptr
						? PlannedMemberBounds(*BlockingMember)
						: FBox(EForceInit::ForceInit);
					const double X = bNormalX ? PostNormal : Station;
					const double Y = bNormalX ? Station : PostNormal;
					const bool bReusableHorizontalCell = BlockingMember != nullptr
						&& BlockingMember->ComponentId == Intent.ComponentId
						&& BlockingMember->Axis != EABTSM73BeamAFrameAxis::Z
						&& X >= BlockingBounds.Min.X - GeometryToleranceCM
						&& X <= BlockingBounds.Max.X + GeometryToleranceCM
						&& Y >= BlockingBounds.Min.Y - GeometryToleranceCM
						&& Y <= BlockingBounds.Max.Y + GeometryToleranceCM
						&& BlockingBounds.Min.Z
							>= SegmentStartZ - GeometryToleranceCM
						&& BlockingBounds.Max.Z
							<= UpperBottomZ + GeometryToleranceCM;
					const bool bReusableClosurePost = BlockingMember != nullptr
						&& BlockingMember->ComponentId == Intent.ComponentId
						&& BlockingMember->SkeletonKind
							== ESkeletonMemberKind::FacadeToTopPost
						&& BlockingMember->Axis == EABTSM73BeamAFrameAxis::Z
						&& X >= BlockingBounds.Min.X - GeometryToleranceCM
						&& X <= BlockingBounds.Max.X + GeometryToleranceCM
						&& Y >= BlockingBounds.Min.Y - GeometryToleranceCM
						...
						&& BlockingBounds.Max.Z
							> SegmentStartZ + GeometryToleranceCM;
					if (bReusableHorizontalCell || bReusableClosurePost)
					{
						if (BlockingBounds.Min.Z
							> SegmentStartZ + GeometryToleranceCM)
						{
							SegmentEndZ = BlockingBounds.Min.Z;
							PostProbe.LocalEnd = PostProbe.LocalStart
								+ FVector(0.0, 0.0,
									SegmentEndZ - SegmentStartZ);
						}
						...
						{
							Connection.ReusedSupportMemberIndices.AddUnique(Blocker);
							Connection.bReusesExistingSupportChain = true;
							LowerMemberIndex = Blocker;
							SegmentStartZ = FMath::Min(
								BlockingBounds.Max.Z, UpperBottomZ);
							continue;
						}
					}
					else
					...
					++Plan.Summary.Stage4FacadeToTopConflictCount;
					OutError = FString::Printf(
						TEXT("BeamC3V3Stage4FacadeToTopPostConflict:Intent=%d:Frame=%d:Station=%.3f:Z=%.3f..%.3f:Blocker=%d:Kind=%d:Axis=%d:Course=%d:ExteriorFrame=%d:ExteriorColumn=%d:TopFrame=%d"),
						Intent.IntentId, Intent.ExteriorFrameId, Station,
						SegmentStartZ, SegmentEndZ, Blocker,
						Plan.Members.IsValidIndex(Blocker)
							? static_cast<int32>(Plan.Members[Blocker].SkeletonKind)
							: INDEX_NONE,
						Plan.Members.IsValidIndex(Blocker)
							? static_cast<int32>(Plan.Members[Blocker].Axis) : INDEX_NONE,
						Plan.Members.IsValidIndex(Blocker)
							? Plan.Members[Blocker].CourseIndex : INDEX_NONE,
						Plan.Members.IsValidIndex(Blocker)
							? Plan.Members[Blocker].ExteriorFrameId : INDEX_NONE,
						Plan.Members.IsValidIndex(Blocker)
							? Plan.Members[Blocker].ExteriorColumnId : INDEX_NONE,
						Plan.Members.IsValidIndex(Blocker)
							? Plan.Members[Blocker].TopSurfaceFrameSegmentId : INDEX_NONE);
					return false;
					}
				}
				int32 MemberIndex = INDEX_NONE;
				if (!AddConnectionMember(ESkeletonMemberKind::FacadeToTopPost,
					...
					FMath::RoundToInt(Station / BlockUnitsCM), INDEX_NONE,
					Intent.FaceMask, EABTSM73BeamAFrameAxis::Z,
					PostProbe.LocalStart, PostProbe.LocalEnd, MemberIndex))
				{
					return false;
				}
				AddSeat(Plan, MemberIndex, LowerMemberIndex);
				Connection.PostSegmentMemberIndices.Add(MemberIndex);
				LowerMemberIndex = MemberIndex;
				SegmentStartZ = SegmentEndZ;
				++Plan.Summary.Stage4FacadeToTopPostSegmentCount;
			}
	...
	TSet<int32> BoundIntentIds;
	FString Canonical = FString::Printf(TEXT("TopFrames=%lld"),
		Plan.Summary.Stage4TopFrameHash);
	for (const FFacadeToTopConnectionPlan& Connection : Plan.FacadeToTopConnections)
	{
		const bool bHasPhysicalPath = Connection.bReusesGroundedAlias
			|| Connection.bDirectFrameContact
			|| Connection.bReusesExistingSupportChain
			|| Connection.SeatMemberIndex != INDEX_NONE
			|| !Connection.PostSegmentMemberIndices.IsEmpty();
		const bool bValidUpper =
			Plan.Members.IsValidIndex(Connection.UpperExteriorFrameMemberIndex);
		if (!bValidUpper || !bHasPhysicalPath)
		{
			++Plan.Summary.Stage4FacadeToTopBindingViolationCount;
			BindingDiagnostics += FString::Printf(
				TEXT("|Connection%d:Upper=%d:Valid=%d:Path=%d:Intents="),
				Connection.ConnectionId, Connection.UpperExteriorFrameMemberIndex,
				bValidUpper ? 1 : 0, bHasPhysicalPath ? 1 : 0);
			for (const int32 IntentId : Connection.SourceIntentIds)
			{
				BindingDiagnostics += FString::Printf(TEXT("%d,"), IntentId);
			}
		}
		Canonical += FString::Printf(
			TEXT("|FT:%d:C%d:EF%d:EM%d:TF%d:TM%d:A%d:T%lld:S%d:SR%d:D%d:R%d:G%d:I"),
			Connection.ConnectionId, Connection.ComponentId,
			Connection.UpperExteriorFrameId,
			Connection.UpperExteriorFrameMemberIndex,
			Connection.TopSurfaceFrameSegmentId,
			Connection.TopSurfaceFrameMemberIndex,
			static_cast<int32>(Connection.TopSurfaceAuthority),
			QHash(Connection.TangentStationCM), Connection.SeatMemberIndex,
			Connection.bReusesExistingSeat ? 1 : 0,
			Connection.bDirectFrameContact ? 1 : 0,
			Connection.bReusesExistingSupportChain ? 1 : 0,
			Connection.bReusesGroundedAlias ? 1 : 0);
		for (const int32 IntentId : Connection.SourceIntentIds)
		{
			BoundIntentIds.Add(IntentId);
			Canonical += FString::Printf(TEXT(",%d"), IntentId);
		...
		{
			Canonical += FString::Printf(TEXT(":P%d"), MemberIndex);
		}
		for (const int32 MemberIndex : Connection.ReusedSupportMemberIndices)
		{
			Canonical += FString::Printf(TEXT(":R%d"), MemberIndex);
		}
		for (const int32 MemberIndex
			: Connection.SuppressedStage3ColumnMemberIndices)
		{
			Canonical += FString::Printf(TEXT(":X%d"), MemberIndex);
		}
	}
	for (const FTopSurfaceIntentPlan& Intent : Plan.TopSurfaceIntents)
	{
			...
			&& !BoundIntentIds.Contains(Intent.IntentId))
		{
			++Plan.Summary.Stage4FacadeToTopBindingViolationCount;
			BindingDiagnostics += FString::Printf(
				TEXT("|UnboundIntent%d:Frame=%d"), Intent.IntentId,
				Intent.ExteriorFrameId);
		}
	}
	Plan.Summary.Stage4FacadeToTopConnectionCount =
		Plan.FacadeToTopConnections.Num();
	if (Plan.Summary.Stage4FacadeToTopBindingViolationCount != 0
		|| Plan.Summary.Stage4FacadeToTopConflictCount != 0)
	{
		OutError = FString::Printf(
			TEXT("BeamC3V3Stage4FacadeToTopContractFailed:Top=%d:Connections=%d:Seats=%d:Posts=%d:Suppressed=%d:ResolvedDeferred=%d:BindingViolations=%d:Conflicts=%d:Diagnostics=%s"),
			Plan.Summary.Stage4ResolvedTopSurfaceIntentCount,
			Plan.Summary.Stage4FacadeToTopConnectionCount,
			Plan.Summary.Stage4FacadeToTopSeatCount,
			Plan.Summary.Stage4FacadeToTopPostSegmentCount,
	...
	Plan.Summary.Stage4FacadeToTopHash = HashText(Canonical);
	Plan.Summary.FinalGeometryHash = HashText(FString::Printf(
		TEXT("Stage3=%lld:Stage4TopFrames=%lld:FacadeToTop=%lld"),
		Plan.Summary.Stage3PlanHash, Plan.Summary.Stage4TopFrameHash,
		Plan.Summary.Stage4FacadeToTopHash));
	Plan.Summary.PlannedMemberCount = Plan.Members.Num();
	if (!ValidatePreflightCaps(Profile.BeamSettings.BeamB.BeamA, Plan, OutError))
	{
		return false;
	...
	for (int32 MemberIndex = InputMemberCount;
		MemberIndex < Plan.Members.Num(); ++MemberIndex)
	{
		const FPlannedMember& Planned = Plan.Members[MemberIndex];
		if (!Assembly.Assemblies.IsValidIndex(Planned.ComponentId))
		{
			OutError = FString::Printf(
				TEXT("BeamC3V3Stage4FacadeToTopInvalidOwner:Member=%d:Component=%d"),
				MemberIndex, Planned.ComponentId);
			return false;
		}
		FABTSM73BeamAMember& Member = Assembly.Members.AddDefaulted_GetRef();
	...
	{
		OutError = FString::Printf(
			TEXT("BeamC3V3Stage4FacadeToTopIRMismatch:Joints=%d/%d:Members=%d/%d"),
			Assembly.Joints.Num(), Plan.Summary.PlannedJointCount,
			Assembly.Members.Num(), Plan.Members.Num());
		return false;
	}
	...
	Plan.Summary.EmittedMemberCount = Assembly.Members.Num();
	Plan.Summary.bAccepted = true;
	Plan.Summary.Stage4FacadeToTopMilliseconds = ElapsedMilliseconds(StartSeconds);
	return Plan.Summary.Stage4FacadeToTopHash != 0;
}

bool FABTSM73BeamC3V3SkeletonFirstGenerator::GenerateStage4FloorStyleInfill(
	const FABTSM73BeamD0ResolvedProfile& Profile,
	const FABTSM73DAG5BV2GenerationResult& Silhouette,
	ABTSM73BeamC3V3::FGenerationResult& OutResult,
	FString& OutError) const
{
	using namespace ABTSM73BeamC3V3;
	const double StartSeconds = FPlatformTime::Seconds();
	if (!GenerateStage4FacadeToTopConnections(
		Profile, Silhouette, OutResult, OutError))
	{
		return false;
	}
	FPlan& Plan = OutResult.Plan;
	FABTSM73BeamAGenerationResult& Assembly = OutResult.Assembly;
	const int32 InputMemberCount = Plan.Members.Num();
	Plan.FloorStyleInfillSpans.Reset();
	Plan.Summary.Stage4FloorSupportPairCount = 0;
	Plan.Summary.Stage4FloorSpanCount = 0;
	Plan.Summary.Stage4StyleInfillSpanCount = 0;
	Plan.Summary.Stage4ReusedFloorSpanCount = 0;
	Plan.Summary.Stage4DeferredFloorSpanCount = 0;
	Plan.Summary.Stage4FloorBindingViolationCount = 0;
	Plan.Summary.Stage4FloorConflictCount = 0;
	Plan.Summary.Stage4RoofReservedMemberCount = 0;
	Plan.Summary.Stage4FloorStyleInfillHash = 0;
	Plan.Summary.Stage4FloorStyleInfillMilliseconds = 0.0;
	...
	struct FSupportPair
	{
		...
		double TangentMinimumCM = 0.0;
		double TangentMaximumCM = 0.0;
		int32 NegativeMemberIndex = INDEX_NONE;
		int32 PositiveMemberIndex = INDEX_NONE;
	};

	auto PlaneKey = [](const int32 ComponentId, const int32 CourseIndex)
	{
		return FString::Printf(TEXT("%d:%d"), ComponentId, CourseIndex);
	};
	auto SupportKey = [](const int32 ComponentId, const int32 CourseIndex,
		const EABTSM73BeamAFrameAxis Axis)
	{
		return FString::Printf(TEXT("%d:%d:%d"), ComponentId, CourseIndex,
			static_cast<int32>(Axis));
	};
	TSet<FString> EligiblePlanes;
	for (const FCommonExteriorFramePlan& Frame : Plan.CommonExteriorFrames)
	{
		EligiblePlanes.Add(PlaneKey(Frame.ComponentId, Frame.CourseIndex));
	}
	for (const FTopSurfaceFrameSegmentPlan& Segment : Plan.TopSurfaceFrameSegments)
	{
		EligiblePlanes.Add(PlaneKey(Segment.ComponentId, Segment.SurfaceCourseIndex));
	}

	TMap<int32, TArray<FBox>> AllowedBoxesByComponent;
	for (const FResolvedFacadeEnvelopeVolume& Volume
		: Plan.ResolvedFacadeEnvelopeVolumes)
	{
		if (Volume.LocalBounds.IsValid)
		{
			AllowedBoxesByComponent.FindOrAdd(Volume.ComponentId).Add(Volume.LocalBounds);
		}
	}

	TMap<FString, TArray<FSupportLine>> SupportGroups;
	for (int32 MemberIndex = 0; MemberIndex < InputMemberCount; ++MemberIndex)
	{
		const FPlannedMember& Member = Plan.Members[MemberIndex];
		if (Member.bSuppressedByStage4FacadeToTop
			|| Member.Axis == EABTSM73BeamAFrameAxis::Z
			|| Member.CourseIndex == INDEX_NONE
			|| !EligiblePlanes.Contains(PlaneKey(
				Member.ComponentId, Member.CourseIndex)))
		{
			continue;
		}
		const int32 UpperCourse = Member.CourseIndex + 1;
		const EABTSM73BeamAFrameAxis TargetAxis = (UpperCourse & 1) == 0
			? EABTSM73BeamAFrameAxis::X : EABTSM73BeamAFrameAxis::Y;
		const EABTSM73BeamAFrameAxis RequiredSupportAxis =
			TargetAxis == EABTSM73BeamAFrameAxis::X
				? EABTSM73BeamAFrameAxis::Y : EABTSM73BeamAFrameAxis::X;
		if (Member.Axis != RequiredSupportAxis)
		{
			continue;
		}
		const FBox Bounds = PlannedMemberBounds(Member);
		const int32 TargetAxisIndex = static_cast<int32>(TargetAxis);
		const int32 TangentAxisIndex = TargetAxisIndex == 0 ? 1 : 0;
		FSupportLine& Line = SupportGroups.FindOrAdd(SupportKey(
			Member.ComponentId, Member.CourseIndex, RequiredSupportAxis))
			.AddDefaulted_GetRef();
		Line.MemberIndex = MemberIndex;
		Line.StationCM = Bounds.GetCenter()[TargetAxisIndex];
		Line.TangentMinimumCM = Bounds.Min[TangentAxisIndex];
		Line.TangentMaximumCM = Bounds.Max[TangentAxisIndex];
	}

	TArray<FString> SortedSupportKeys;
	SupportGroups.GetKeys(SortedSupportKeys);
	SortedSupportKeys.Sort();
	TArray<FSupportPair> SupportPairs;
	for (const FString& Key : SortedSupportKeys)
	{
		TArray<FSupportLine>& Lines = SupportGroups.FindChecked(Key);
		Lines.Sort([](const FSupportLine& A, const FSupportLine& B)
		{
			return !FMath::IsNearlyEqual(A.StationCM, B.StationCM,
				GeometryToleranceCM)
				? A.StationCM < B.StationCM : A.MemberIndex < B.MemberIndex;
		});
		TArray<double> Stations;
		for (const FSupportLine& Line : Lines)
		{
			if (Stations.IsEmpty() || !FMath::IsNearlyEqual(
				Stations.Last(), Line.StationCM, GeometryToleranceCM))
			{
				Stations.Add(Line.StationCM);
			}
		}
		for (int32 StationIndex = 0; StationIndex + 1 < Stations.Num(); ++StationIndex)
		{
			const double Minimum = Stations[StationIndex];
			const double Maximum = Stations[StationIndex + 1];
			const double SpanLength = Maximum - Minimum;
			if (!IsIntegerMultipleOfBlock(SpanLength)
				|| SpanLength < BlockUnitsCM - GeometryToleranceCM
				|| SpanLength > MaximumHorizontalUnits * BlockUnitsCM
					+ GeometryToleranceCM)
			{
				++Plan.Summary.Stage4DeferredFloorSpanCount;
				continue;
			}
			const FSupportLine* BestNegative = nullptr;
			const FSupportLine* BestPositive = nullptr;
			double BestOverlapMinimum = 0.0;
			double BestOverlapMaximum = 0.0;
			double BestOverlap = 0.0;
			for (const FSupportLine& Negative : Lines)
			{
				if (!FMath::IsNearlyEqual(Negative.StationCM, Minimum,
					GeometryToleranceCM))
				{
					continue;
				}
				for (const FSupportLine& Positive : Lines)
				{
					if (!FMath::IsNearlyEqual(Positive.StationCM, Maximum,
						GeometryToleranceCM))
					{
						continue;
					}
					const double OverlapMinimum = FMath::Max(
						Negative.TangentMinimumCM, Positive.TangentMinimumCM);
					const double OverlapMaximum = FMath::Min(
						Negative.TangentMaximumCM, Positive.TangentMaximumCM);
					const double Overlap = OverlapMaximum - OverlapMinimum;
					if (Overlap > BestOverlap + GeometryToleranceCM
						|| (FMath::IsNearlyEqual(Overlap, BestOverlap,
							GeometryToleranceCM)
							&& (BestNegative == nullptr
								|| Negative.MemberIndex < BestNegative->MemberIndex
								|| (Negative.MemberIndex == BestNegative->MemberIndex
									&& Positive.MemberIndex
										< BestPositive->MemberIndex))))
					{
						BestNegative = &Negative;
						BestPositive = &Positive;
						BestOverlapMinimum = OverlapMinimum;
						BestOverlapMaximum = OverlapMaximum;
						BestOverlap = Overlap;
					}
				}
			}
			if (BestNegative == nullptr || BestPositive == nullptr
				|| BestOverlap < BlockUnitsCM - GeometryToleranceCM)
			{
				++Plan.Summary.Stage4DeferredFloorSpanCount;
				continue;
			}
			const FPlannedMember& NegativeMember =
				Plan.Members[BestNegative->MemberIndex];
			FSupportPair& Pair = SupportPairs.AddDefaulted_GetRef();
			Pair.ComponentId = NegativeMember.ComponentId;
			Pair.LowerCourseIndex = NegativeMember.CourseIndex;
			Pair.Axis = NegativeMember.Axis == EABTSM73BeamAFrameAxis::Y
				? EABTSM73BeamAFrameAxis::X : EABTSM73BeamAFrameAxis::Y;
			Pair.MinimumCM = Minimum;
			Pair.MaximumCM = Maximum;
			Pair.TangentMinimumCM = BestOverlapMinimum;
			Pair.TangentMaximumCM = BestOverlapMaximum;
			Pair.NegativeMemberIndex = BestNegative->MemberIndex;
			Pair.PositiveMemberIndex = BestPositive->MemberIndex;
		}
	}
	SupportPairs.Sort([](const FSupportPair& A, const FSupportPair& B)
	{
		if (A.ComponentId != B.ComponentId) return A.ComponentId < B.ComponentId;
		if (A.LowerCourseIndex != B.LowerCourseIndex)
			return A.LowerCourseIndex < B.LowerCourseIndex;
		if (A.Axis != B.Axis)
			return static_cast<int32>(A.Axis) < static_cast<int32>(B.Axis);
		if (!FMath::IsNearlyEqual(A.MinimumCM, B.MinimumCM,
			GeometryToleranceCM)) return A.MinimumCM < B.MinimumCM;
		return A.TangentMinimumCM < B.TangentMinimumCM;
	});

	const int32 MaximumMemberCount =
		Profile.BeamSettings.BeamB.BeamA.MaxMemberCount;
	const int32 RemainingBeforeInfill = FMath::Max(
		0, MaximumMemberCount - InputMemberCount);
	const int32 DesiredRoofReserve = FMath::Max(24,
		Silhouette.Summary.PyramidCount * 4
			+ Silhouette.Summary.PrismCount * 2);
	const int32 RoofReserve = FMath::Min(
	...
	const uint32 StylePhaseSeed = FCrc::StrCrc32(*StableStyleProfileId);

	auto FindCoverOrConflict = [&Plan](const FPlannedMember& Probe,
		int32& OutCoveringMember, int32& OutConflictMember)
	{
		OutCoveringMember = INDEX_NONE;
		OutConflictMember = INDEX_NONE;
		const FBox Bounds = PlannedMemberBounds(Probe);
		for (int32 MemberIndex = 0; MemberIndex < Plan.Members.Num(); ++MemberIndex)
		{
			const FPlannedMember& Existing = Plan.Members[MemberIndex];
			if (Existing.bSuppressedByStage4FacadeToTop)
			{
				continue;
			}
			const FBox ExistingBounds = PlannedMemberBounds(Existing);
			...
			if (Existing.Axis == Probe.Axis
				&& Bounds.Min.X >= ExistingBounds.Min.X - GeometryToleranceCM
				&& Bounds.Min.Y >= ExistingBounds.Min.Y - GeometryToleranceCM
				&& Bounds.Min.Z >= ExistingBounds.Min.Z - GeometryToleranceCM
				&& Bounds.Max.X <= ExistingBounds.Max.X + GeometryToleranceCM
				&& Bounds.Max.Y <= ExistingBounds.Max.Y + GeometryToleranceCM
				&& Bounds.Max.Z <= ExistingBounds.Max.Z + GeometryToleranceCM)
			{
				OutCoveringMember = MemberIndex;
				return;
			}
			OutConflictMember = MemberIndex;
			return;
		}
	};

	FString Canonical = FString::Printf(TEXT("FacadeToTop=%lld:RoofReserve=%d"),
		Plan.Summary.Stage4FacadeToTopHash, RoofReserve);
	FString BindingDiagnostics;
	for (const FSupportPair& Pair : SupportPairs)
	{
		const int32 UpperCourse = Pair.LowerCourseIndex + 1;
		const double GroundZ = Plan.Components.IsValidIndex(Pair.ComponentId)
			? Plan.Components[Pair.ComponentId].GroundPlaneZCM : 0.0;
		const double Z = GroundZ + (UpperCourse + 0.5) * BlockUnitsCM;
		const double FirstStation = Pair.TangentMinimumCM + BlockUnitsCM * 0.5;
		const double LastStation = Pair.TangentMaximumCM - BlockUnitsCM * 0.5;
		if (LastStation < FirstStation - GeometryToleranceCM)
		{
			++Plan.Summary.Stage4DeferredFloorSpanCount;
			continue;
		}
		TArray<double> Stations;
		for (double Station = FirstStation;
			Station <= LastStation + GeometryToleranceCM; Station += BlockUnitsCM)
		{
			Stations.Add(Station);
		}
		if (Stations.IsEmpty())
		...
		const double Midpoint = (Pair.TangentMinimumCM
			+ Pair.TangentMaximumCM) * 0.5;
		int32 PrimaryStationIndex = 0;
		for (int32 Index = 1; Index < Stations.Num(); ++Index)
		{
			if (FMath::Abs(Stations[Index] - Midpoint)
				< FMath::Abs(Stations[PrimaryStationIndex] - Midpoint)
					- GeometryToleranceCM)
			{
				PrimaryStationIndex = Index;
			}
		}
		TArray<int32> CandidateOrder;
		CandidateOrder.Add(PrimaryStationIndex);
		for (int32 Index = 0; Index < Stations.Num(); ++Index)
		{
			if (Index != PrimaryStationIndex)
			{
				CandidateOrder.Add(Index);
			}
		}
		bool bPrimaryResolved = false;
		bool bPairCounted = false;
		int32 CoveredCandidateCount = 0;
		int32 SemanticRejectedCandidateCount = 0;
		int32 ConflictCandidateCount = 0;
		int32 ReusedCandidateCount = 0;
		int32 LastConflictMember = INDEX_NONE;
		for (const int32 StationIndex : CandidateOrder)
		{
			const bool bPreferredPrimaryCandidate =
				StationIndex == PrimaryStationIndex;
			const bool bStyleSelected = !bPreferredPrimaryCandidate
				&& StyleStride != MAX_int32
				&& ((StationIndex + static_cast<int32>(StylePhaseSeed % StyleStride))
					% StyleStride) == 0;
			const bool bNeedsPrimary = !bPrimaryResolved;
			if (!bNeedsPrimary && !bStyleSelected)
			{
				continue;
			}
			// The geometric midpoint is preferred, not mandatory. If it is occupied
			// or outside the semantic union, the first later legal lane becomes the
			// main floor. This prevents a valid support pair from being represented
			// only by optional StyleInfill rows.
			const bool bStyle = !bNeedsPrimary;
			const double Tangent = Stations[StationIndex];
			FVector Start = FVector::ZeroVector;
			FVector End = FVector::ZeroVector;
			...
			Start.Z = End.Z = Z;
			FPlannedMember Probe;
			Probe.Axis = Pair.Axis;
			Probe.LocalStart = Start;
			Probe.LocalEnd = End;
			const FBox ProbeBounds = PlannedMemberBounds(Probe);
			const TArray<FBox>* AllowedBoxes =
				AllowedBoxesByComponent.Find(Pair.ComponentId);
			FVector UncoveredPoint = FVector::ZeroVector;
			const bool bCovered = AllowedBoxes != nullptr
				&& SolidCoveredByBoxes(ProbeBounds, *AllowedBoxes, UncoveredPoint);
			const bool bVoidBlocked = Plan.ReservedSupportVoids.ContainsByPredicate(
				[&ProbeBounds](const FABTSM73BeamASupportVoid& Void)
				{
					return BoxesPenetrate(ProbeBounds, Void.Bounds);
				});
			if (!bCovered || bVoidBlocked)
			{
			...
			int32 CoveringMember = INDEX_NONE;
			int32 ConflictMember = INDEX_NONE;
			FindCoverOrConflict(Probe, CoveringMember, ConflictMember);
			if (ConflictMember != INDEX_NONE)
			{
				++ConflictCandidateCount;
				LastConflictMember = ConflictMember;
				++Plan.Summary.Stage4DeferredFloorSpanCount;
				continue;
			}
			// A support pair becomes a binding Floor obligation only after at
			// least one semantically contained lane survives the immutable-prefix
			// collision test. Fully occupied lanes remain explicit deferred work;
			// they must not force a new member through an existing core or post.
			if (!bPairCounted)
			{
				++Plan.Summary.Stage4FloorSupportPairCount;
				bPairCounted = true;
			}
			if (CoveringMember == INDEX_NONE
				&& Plan.Members.Num() >= InfillMemberCeiling)
			{
				if (bStyle)
				{
					++Plan.Summary.Stage4DeferredFloorSpanCount;
				...
				++Plan.Summary.Stage4FloorBindingViolationCount;
				OutError = FString::Printf(
					TEXT("BeamC3V3Stage4FloorBudgetInsufficient:Members=%d:Ceiling=%d:RoofReserve=%d:Pair=%d/%d:Course=%d"),
					Plan.Members.Num(), InfillMemberCeiling, RoofReserve,
					Pair.NegativeMemberIndex, Pair.PositiveMemberIndex, UpperCourse);
				return false;
			}
			const int32 SpanId = Plan.FloorStyleInfillSpans.Num();
			int32 SourceVolumeId = INDEX_NONE;
			if (const TArray<FBox>* ComponentBoxes =
				AllowedBoxesByComponent.Find(Pair.ComponentId))
			{
				const FVector Center = ProbeBounds.GetCenter();
				for (const FResolvedFacadeEnvelopeVolume& Volume
					: Plan.ResolvedFacadeEnvelopeVolumes)
				{
					if (Volume.ComponentId == Pair.ComponentId
						&& Volume.LocalBounds.IsInsideOrOn(Center))
					{
						SourceVolumeId = Volume.SourceVolumeId;
						break;
					}
				}
				(void)ComponentBoxes;
			}
			int32 MemberIndex = CoveringMember;
			if (MemberIndex == INDEX_NONE)
			{
				if (!AddPlannedMember(Plan,
					bStyle ? EOwnerKind::StyleInfill : EOwnerKind::Floor,
					bStyle ? ESkeletonMemberKind::StyleInfillCourse
						: ESkeletonMemberKind::FloorInfillCourse,
					SpanId, Pair.ComponentId, SourceVolumeId, INDEX_NONE,
					UpperCourse, FMath::RoundToInt(Pair.MinimumCM / BlockUnitsCM),
					FMath::RoundToInt(Tangent / BlockUnitsCM), 0, Pair.Axis,
					bStyle ? EABTSM73BeamAMemberRole::SecondaryBeam
						: EABTSM73BeamAMemberRole::PrimaryBeam,
					Start, End, OutError))
				{
					return false;
				}
				MemberIndex = Plan.Members.Num() - 1;
				Plan.Members[MemberIndex].FloorStyleInfillSpanId = SpanId;
				AddSeat(Plan, MemberIndex, Pair.NegativeMemberIndex);
				AddSeat(Plan, MemberIndex, Pair.PositiveMemberIndex);
			}
			else
			{
				++ReusedCandidateCount;
				++Plan.Summary.Stage4ReusedFloorSpanCount;
			}
			FFloorStyleInfillSpanPlan& Span =
				Plan.FloorStyleInfillSpans.AddDefaulted_GetRef();
			Span.SpanId = SpanId;
			Span.ComponentId = Pair.ComponentId;
			Span.SourceVolumeId = SourceVolumeId;
			Span.LowerCourseIndex = Pair.LowerCourseIndex;
			Span.CourseIndex = UpperCourse;
			Span.Axis = Pair.Axis;
			Span.TangentCoordinateCM = Tangent;
			Span.MinimumCM = Pair.MinimumCM;
			Span.MaximumCM = Pair.MaximumCM;
			Span.NegativeSupportMemberIndex = Pair.NegativeMemberIndex;
			Span.PositiveSupportMemberIndex = Pair.PositiveMemberIndex;
			Span.MemberIndex = MemberIndex;
			Span.bStyleInfill = bStyle;
			Span.bReusesExistingMember = CoveringMember != INDEX_NONE;
			if (bStyle)
			{
				++Plan.Summary.Stage4StyleInfillSpanCount;
			}
			else
			{
				++Plan.Summary.Stage4FloorSpanCount;
				bPrimaryResolved = true;
			}
			Canonical += FString::Printf(
				TEXT("|FI:%d:C%d:L%d:U%d:A%d:T%lld:R%lld..%lld:N%d:P%d:M%d:S%d:E%d"),
				SpanId, Pair.ComponentId, Pair.LowerCourseIndex, UpperCourse,
				static_cast<int32>(Pair.Axis), QHash(Tangent),
				QHash(Pair.MinimumCM), QHash(Pair.MaximumCM),
				Pair.NegativeMemberIndex, Pair.PositiveMemberIndex, MemberIndex,
				bStyle ? 1 : 0, CoveringMember != INDEX_NONE ? 1 : 0);
		}
		if (bPairCounted && !bPrimaryResolved)
		{
			++Plan.Summary.Stage4FloorBindingViolationCount;
			FString BlockerDiagnostic = TEXT("None");
			if (Plan.Members.IsValidIndex(LastConflictMember))
			{
				const FPlannedMember& Blocker = Plan.Members[LastConflictMember];
				const FBox BlockerBounds = PlannedMemberBounds(Blocker);
				BlockerDiagnostic = FString::Printf(
					TEXT("M%d/K%d/O%d/A%d/L%d/B%.1f,%.1f,%.1f..%.1f,%.1f,%.1f"),
					LastConflictMember, static_cast<int32>(Blocker.SkeletonKind),
					static_cast<int32>(Blocker.OwnerKind),
					static_cast<int32>(Blocker.Axis), Blocker.CourseIndex,
					BlockerBounds.Min.X, BlockerBounds.Min.Y, BlockerBounds.Min.Z,
					BlockerBounds.Max.X, BlockerBounds.Max.Y, BlockerBounds.Max.Z);
			}
			BindingDiagnostics += FString::Printf(
				TEXT("|Pair:C%d:L%d:A%d:R%.1f..%.1f:T%.1f..%.1f:N%d:P%d:Covered%d:SemanticRejected%d:Conflicts%d:Reused%d:Blocker=%s"),
				Pair.ComponentId, Pair.LowerCourseIndex,
				static_cast<int32>(Pair.Axis), Pair.MinimumCM, Pair.MaximumCM,
				Pair.TangentMinimumCM, Pair.TangentMaximumCM,
				Pair.NegativeMemberIndex, Pair.PositiveMemberIndex,
				CoveredCandidateCount, SemanticRejectedCandidateCount,
				ConflictCandidateCount, ReusedCandidateCount,
				*BlockerDiagnostic);
		}
	}

	if (Plan.Summary.Stage4FloorBindingViolationCount != 0
		|| Plan.Summary.Stage4FloorConflictCount != 0)
	{
		OutError = FString::Printf(
			TEXT("BeamC3V3Stage4FloorStyleInfillContractFailed:Pairs=%d:Floor=%d:Style=%d:Reused=%d:Deferred=%d:Binding=%d:Conflicts=%d:RoofReserve=%d:Diagnostics=%s"),
			Plan.Summary.Stage4FloorSupportPairCount,
			Plan.Summary.Stage4FloorSpanCount,
			Plan.Summary.Stage4StyleInfillSpanCount,
			Plan.Summary.Stage4ReusedFloorSpanCount,
			Plan.Summary.Stage4DeferredFloorSpanCount,
			Plan.Summary.Stage4FloorBindingViolationCount,
			Plan.Summary.Stage4FloorConflictCount, RoofReserve,
			*BindingDiagnostics);
		return false;
	}
	Plan.Summary.Stage4FloorStyleInfillHash = HashText(Canonical);
	Plan.Summary.FinalGeometryHash = HashText(FString::Printf(
		TEXT("FacadeToTop=%lld:FloorStyleInfill=%lld"),
		Plan.Summary.Stage4FacadeToTopHash,
		Plan.Summary.Stage4FloorStyleInfillHash));
	Plan.Summary.PlannedMemberCount = Plan.Members.Num();
	if (!ValidatePreflightCaps(Profile.BeamSettings.BeamB.BeamA, Plan, OutError))
	{
		return false;
		...
		MemberIndex < Plan.Members.Num(); ++MemberIndex)
	{
		...
		{
			OutError = FString::Printf(
				TEXT("BeamC3V3Stage4FloorInvalidOwner:Member=%d:Component=%d"),
				MemberIndex, Planned.ComponentId);
			return false;
		}
		FABTSM73BeamAMember& Member = Assembly.Members.AddDefaulted_GetRef();
	...
	{
		OutError = FString::Printf(
			TEXT("BeamC3V3Stage4FloorIRMismatch:Joints=%d/%d:Members=%d/%d"),
			Assembly.Joints.Num(), Plan.Summary.PlannedJointCount,
			Assembly.Members.Num(), Plan.Members.Num());
		return false;
	}
	...
	Plan.Summary.EmittedMemberCount = Assembly.Members.Num();
	Plan.Summary.bAccepted = true;
	Plan.Summary.Stage4FloorStyleInfillMilliseconds =
		ElapsedMilliseconds(StartSeconds);
	return Plan.Summary.Stage4FloorStyleInfillHash != 0;
}
