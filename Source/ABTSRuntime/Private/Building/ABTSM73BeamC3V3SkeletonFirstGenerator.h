...
public:
	...
	/** Preserves Stage 3 exactly and classifies every exterior frame as a real
	 * ground-sill or top-surface closure. Emits no Stage-4 member geometry. */
	bool GenerateStage4TopSurfaceIntent(
		const FABTSM73BeamD0ResolvedProfile& Profile,
		const FABTSM73DAG5BV2GenerationResult& Silhouette,
		ABTSM73BeamC3V3::FGenerationResult& OutResult,
		FString& OutError) const;
	...
	/** Replaces temporary Stage-3 downward columns for TopSurface-owned facades
	 * with traceable seats/posts rooted in the accepted top-frame ledger. */
	bool GenerateStage4FacadeToTopConnections(
		const FABTSM73BeamD0ResolvedProfile& Profile,
		const FABTSM73DAG5BV2GenerationResult& Silhouette,
		ABTSM73BeamC3V3::FGenerationResult& OutResult,
		FString& OutError) const;
